...
- **The Customer is the Service module's pre-estimate record, in its own view** (D-075, 2026-09-23, `docs/service-customer-layout.md`): `/service/customers` lists the customers whose `Customer_Type__c` includes `Service` (`GET /sf/customer … &op=includes`), as a table or a board by **`Service_Stage__c`** — the Service pipeline's OWN field (New → … → Resolved / Closed; `Stage__c` stays the solar sales pipeline, `Status__c` stays the hub lifecycle), with `Service_Request_Type__c`, `Service_Resolution__c`, `Service_Resolved_Date__c` (package `salesforce/service-customer-2026-09-23/`). The page is the SAME record the Sales page edits (same full read, PATCH, edit engine, Files, Comments) with the Service subset of fields; Create Estimate / Create Job pre-pick the customer; `POST /service/customers` is the standalone New Customer / Add to Service (select-or-create + `Service_Stage__c = New`, never overwriting a stage). Assigned To (`Assigned_To__c`) is shown as a NAME joined from the user list, never an id. Reminders are Tim's Flows, never the portal.
- **The brand on every customer document comes from Secrets Manager `sundial/brand`** (2026-09-24, `lib/brand.js`): per tenant slug — logo (`logoUrl`, shown on the hosted page, embedded in the PDF), identity lines, and the footer's **Terms & Conditions** and **Solar Service Club** links with their copy — on the estimate, invoice and job report alike. One `brandBlock()` feeds the three builders; never a logo, a URL or a blurb in code.
