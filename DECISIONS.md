...
### Phase 1b outcome (2026-08-27) — the lesson repeated one layer down, and a false green

Phase 1b shipped `sql/sundial_access_p1b_comment_rls.sql` (Parts A/B/C) and the §3.7 re-check in `sundial-comment-notify`. It is **the first change in this design that alters what a live user sees**: Dennis goes from all 511 tenant comments to the 79 on his own records, keeping all 26 he authored; 23 tenant-scope users are unchanged; 14 of 14 mention rows stay readable by their recipient. Gates: 44/44 as the ZZ users, 11/11 on the notify chain, 644 unit tests. Evidence table in `docs/access-model.md` §8.

**Three measurements amended the design as written (A7 in §5.2).** `profiles.access_scope`, which §5.2 named as the only scope source, was **NULL on 21 of 35 rows** — it is written only on a `/auth/me` since the Phase 1 deploy, so "read profiles, deny NULL" would have blacked out 17 Harmon staff including 11 of 14 Executives. Scope is now the **narrower** of that column and a `sundial_user_cache` derivation, nulls ignored; `profiles` keeps authority because it alone knows `Dealer__c.Active__c`, and the cache covers everyone regardless of login. `sundial_user_cache` gained `supabase_user_id` so a mentioned user with no `profiles` row still resolves — needed by exactly one live user, an Executive provisioned but never signed in, who would otherwise have been permanently unmentionable with no symptom but a mention insert that quietly failed. And the `tenant` branch stopped consulting the cache, because 18 commented records (28 comments) are no longer in it and the pseudocode would have hidden them from admins too.

**The Phase 1 lesson repeated itself immediately, one layer down.** Part B did `revoke all on function ... from public` and V2 still reported `anon_exec = true` on all four helpers. Supabase ships `alter default privileges in schema public grant all on functions to anon, authenticated, service_role`, so each function had a **direct** grant per role and the PUBLIC revoke removed an entry that was not doing the work. The tell was `private.resolve_access` coming back correctly locked — those defaults are scoped `in schema public`. Only `record_visible_for` was materially exposed (it takes its subject as an argument, so it never consults `auth.uid()`, making it an unauthenticated boolean oracle), and Part C closes it. **The generalisation above needs one more clause: a revoke from PUBLIC is not a revoke from a role that holds the privilege directly, and on this project the platform re-grants directly by default.**

**The other finding is about verification, not access.** V10–V12 impersonated a test session by resolving its uuid with a subquery placed **after** `set local role authenticated` — where `own_profile_select` hides the row, so `sub` was NULL and the block measured a session that was nobody. It returned `uid null / 0 / 0`, **which is precisely what a correctly-scoped rep with no seeded comments returns.** A verification query that cannot fail is worse than no verification query, because it is recorded as evidence. Caught by Tim running it rather than by anyone reading it. The fix is an ordering swap; the standing rule added to the file is to **assert the identity column is non-null before believing any count beside it** — which generalises to every impersonation-based check this project adds later.
...
### D-072 amendment 7 (2026-09-16): the technician app lives inside the portal at /tech, is online-first with an ordered replay queue, and the clock is an append-only log the tech can never edit

**One portal, one login, one deploy.** The tech app is `/tech` inside `harmon-crm` (its own phone-first `TechLayout`, not `AppLayout`), installable through a manifest + a shell-only service worker registered from `/tech` alone. A Technician login (`lib/access.js` scope **`tech`**, the new `Access_Level__c = Technician` mapping) has exactly one action, **`service.tech.self`**, and nothing else — no `/sf` reads, no modules — so the app's routes are the whole of what a tech can reach, and the office's routes never learn a tech exists. Every office route redirects a Technician to `/tech`; the office can open `/tech` too and look at any tech's day (`techId=`). A separate app, a separate repo, or a native wrapper would each have added a build and a login for no capability the phone's browser lacks.

**Online-first with a queue, not offline-first.** A tap is sent at once when there is signal. When there is not — or the request dies (network, 5xx, 429, 401) — it is queued on the phone with **its tap time and its own id**, painted onto the local copy immediately, and replayed **in order, one at a time** on the next `online` / app open / tab focus. The id is the server's idempotency key (`eventId`), so a lost response never doubles an interval, a note or a text; the tap time is what the server stamps, so a note written in a basement at 9:41 says 9:41. A tap the server refuses (any other 4xx) is kept as *failed* with the server's reason and replay continues — the tech sees it and discards it; it is never retried forever and never silently dropped. Anything already waiting for a call makes the next tap on that call queue behind it. A full offline-first design (a local database as the source of truth, conflict resolution) was rejected: seven techs in Phoenix are rarely without signal for long, and the office's board must be the referee — the queue keeps the phone honest without making it authoritative.

**The clock is a log.** `Clock_Intervals__c` is append-only JSON: "On my way" opens an `en_route` interval (drive time belongs to the destination) and ends whatever the tech had open on another call (an En Route one drops back to Scheduled — they never arrived; an In Progress one stays In Progress, paused); "Clock in" continues it (`arrived`) or opens `on_site`; "Complete" closes it. `Actual_Start__c` = the first `in`, `Actual_End__c` = the last `out`, `Duration_Minutes__c` = the sum — so labor billing (amendment 6) reads the same numbers the office sees. A reopen adds an interval; nothing is ever edited or deleted by the app, and a tap dated before the log's last event is refused (`AT_OUT_OF_ORDER`): time corrections are the office's job on the board. **Completion is gated** by the checklist — one generic default today (notes ✓auto, a photo ✓auto, walk the customer through, pick up your tools; review optional), a per-tenant template library keyed by `Checklist_Template_Key__c` later — and the gate is enforced on the server; the phone only pre-checks it.

**The geofence is a tag.** On clock-in with GPS the job is geocoded once (Google Geocoding on the existing `sundial/google-maps` key — the same account, no AWS Location today — written to `Geocode_*` on the job; `Failed` is remembered, a Google error is retried) and the fix is compared with the job **or** the shop (`SERVICE_SHOP_LATLNG`) within `SERVICE_GEOFENCE_METERS` (250). Within → `Geofence_Verified__c`. It never blocks a tap: no permission, no fix, no key, no geocode all mean "unverified", which is information for the office, not a wall in front of the tech.

**One sender for every customer text.** The "on my way" text goes through **`lib/sms-send.js`**, extracted from `sundial-sms` so that the board Lambda and the office's Communications panel share one code path: same secret, same tenant→number rule, same `sundial_sms_messages` row, same broadcast — a reply matches back to the job by the conversation rule regardless of who started it. No mobile number = no text and a reported reason, never a failed tap. Field lines (`POST /service/tech/calls/{id}/estimate-lines`) stay in the estimate Lambda so the money math has one home: every line a tech adds is **Proposed**, `Source__c = Field`, tagged with the call, for the office to review and the customer to approve as usual; a tech can never approve.

#### Amendment 7 addendum (2026-09-16): a tech can see the whole Service module, read-only, and talk through the same Communications panel as the office

After the first day on a phone Tim asked for three things: team notes with @-tagging (or better, the job's Communications feed), lists of customers / estimates / jobs a tech can look up whether or not the work is theirs, and a menu with room to grow. The scope model gains **`service.tech.read`** (tech + tenant): read-only, tenant-wide routes for jobs, estimates and customers in `tech.js`, and the sms-thread + job-activity reads in the sms / estimate Lambdas accept it alongside `service.estimate.write` (`assertAction` now takes a list). `service.sms.send` extends to a tech — the same number, the same thread, the office sees every message. The user directory (`/sf/users`) serves a tech, because a tech is staff and the directory is the mention list. Writes are unchanged: still only `service.tech.self` on the tech's own calls, plus texting.

Team notes stay **browser-direct under RLS**, as everywhere else in the portal, so the database's copy of the scope table had to learn the word: `sql/sundial_access_p10_tech_scope.sql` gives `private.resolve_access()` a `tech` lane (the cache's "active Technician" or the profile's `tech`, never over a `none`), makes `record_visible_for()` answer true for `job` / `estimate` / `customer` in the tech's tenant and false for everything else, and treats a tech as staff in `user_visible()`. The policies and `mentionable_users()` are untouched — they call these helpers. The tech app then reuses `CommunicationsPanel` itself inside a `.dark` wrapper (CLAUDE.md's rule: change tagging in the shared code, never in a copy).

On the alternative — routing comments through a Lambda for techs — no: it would have been a second write path into `comments` that the RLS-based mention picker, the mentions feed and `sundial-comment-notify` all reason about from the policy side, and a second path is a drift waiting to happen.

**Scope of D-064 (Tim, 2026-09-17).** A parallel session had asked that these changes go through the D-064 design → report → approval process first. Tim's decision: D-064 is the **sales-rep and dealer** sharing model — who may see whose customers and deals — and it does not govern technicians, who are all Harmon employees working inside one tenant. The `tech` scope is a separate lane (`Technician → SCOPES.TECH` in `lib/access.js`, the `tech` lane in `private.resolve_access()`), not a variant of `dealer` / `own`, and future tech-app access changes need no D-064 report. The D-064 rule that *does* carry over unchanged is the structural one: every authorization decision lives in `lib/access.js` and the database helpers that mirror it — never a second implementation in a Lambda or the portal.

### D-072 amendment 8 (2026-09-17): payments are Stripe Checkout + a signed webhook into settleMoney; money that arrives before its job is deferred, never lost

**Context.** D-065 decision 6 set the payment shape (SetupIntent + off-session charge, never auth-and-capture; Harmon's own Stripe account; webhook fail closed) and D-072.7 made the hosted estimate page the payment link. Tim got a user on Harmon's Stripe account on 2026-09-17, so this is the build.

**Decision.**
1. **The customer pays on Stripe's page, never ours.** `POST /public/estimates/{token}/checkout` mints a Stripe **Checkout Session** — `setup` mode to keep a card on file, `payment` mode (with `setup_future_usage: off_session`) for the deposit or the invoice balance — and the portal sends the customer to it. No card field, no Stripe.js, no publishable key in the browser: the page only ever holds a Checkout URL. The step offered is **re-derived from the records on the server** (`paymentSummary`); the browser's word for it is refused with a 409.
2. **Stripe tells Sundial through one signed webhook** (`POST /webhooks/stripe/{tenant}`, in the estimate Lambda so it reaches `settleMoney` directly). Gate: the `Stripe-Signature` HMAC over the raw body, 5-minute tolerance, constant-time, **503 with no secret** — the same doorbell discipline as `sundial-sms` and the Aurora webhook. The tenant is the URL's slug, cross-checked against the event's metadata and the keys' live/test mode.
3. **The PaymentIntent id is the idempotency key** (`Stripe_Payment_Intent_Id__c`), a refund's id for refunds, and Stripe's event id for the ledger (`sundial_stripe_events`). Every path looks before it writes; a redelivered event, or a webhook that beats the office's own confirm, cannot double-count. The office's off-session charge writes its **Pending row before confirming** so the webhook only ever updates.
4. **Money before the job exists is deferred.** A deposit paid at approval usually precedes Create Job. A Payment row needs a job, so the webhook stamps `Deposit_Paid_At__c`, keeps the event `deferred` in the ledger, and Create Job writes the row (and the card-on-file flag) then. Rejected alternative: quick-creating the job from the webhook — the office chooses when and how a job is created, and a webhook must never create operational records.
5. **Keys are per tenant in `sundial/stripe`** (`tenants[slug]`), read through `lib/stripe.js` — a page of REST over `fetch` rather than the SDK, because five calls do not justify bundling an SDK into three Lambdas, and a signature check we can read is worth more than one we import.
6. **Refunds are made in the Stripe dashboard** and mirrored by `charge.refunded` — no refund button in Sundial today (the office has the dashboard; a Sundial button would be a second place money leaves).

**Consequences.** `Customer_Card_on_File__c` is a mirror of Stripe's default payment method, set by the webhook; `Stripe_Customer_Id__c` lives on the customer hub (per §3.6). Receipts are Stripe's own for now; the "receipt + photo job report" email is the next increment. Stripe's webhook must be subscribed to exactly four event types (runbook). Test-mode and live keys are the same shape; the code refuses a live event on test keys.


### D-072 amendment 9 (2026-09-19): a Complete call's notes roll up onto the job in the Lambda, never by a Flow; `Customer_Type__c` is the department tag

**Context.** Tim added two Long Text Area fields to the job — `Notes_for_Summary__c` and `Notes_From_Service_Calls__c` — and asked for the tech's visit notes to land there: private notes in the second, everything else in the first, appended to what is already there, while the call keeps its own copy. He asked whether a Salesforce Flow would be easier. He also added `Customer_Type__c` (multi-select: Solar / Roofing / Commercial / Service) on the customer hub.

**Decision.**
1. **The roll-up runs in `sundial-service-board/job-notes.js`, at completion, one block per call.** A Flow was rejected on three counts: a record-triggered Flow fires on every save and cannot tell an append from an edit, so a corrected note lands twice; it cannot know the call is *done* (notes are typed all visit long — the office wants the finished visit, not a live stream); and a Flow's write bypasses the job cache and the activity feed. In the Lambda the moment is exact (the same code path that marks the call Complete), the block is keyed by the call number so a later edit replaces it rather than stacking, emptied notes remove it, and the office's own text ahead of the blocks is never touched.
2. **Before completion nothing reaches the job.** A half-written note is not a summary. After completion, every edit to the call's notes (board PATCH, tech note) re-rolls that call's block.
3. **`Customer_Type__c` is set the way `Requested_Project_Types__c` is:** the Sales module's New Customer defaults `Solar` (portal default + a describe-guarded server default in `sundial-sf-update`); the Service module's popups and the Service Club join set `Service` on a new customer and union-add it to an existing one (the estimate Lambda's `resolveCustomer`, same guarded picklist match). The field lives in `salesforce/service-objects/` as a delta on the customer (with the permission set's FLS) so the repo is the record; cache column `customer_type`.

**Consequences.** `GET /service/jobs/{id}/calls` carries each call's clock log with GPS and the job's geocode so the office sees where the tech clocked (ask #1); the tech app reads the job's Street View still and `Initial_Remote_Diagnosis__c` (asks #2, #4) and filters Customers by type (#3). The job cache gains the two notes columns (`sql/2026-09-19_customer_type_job_notes.sql`). The Sales-rep field manifest does not yet list `Customer_Type__c` — a sheet row when reps need to see it.

### D-072 amendment 10 (2026-09-19): the customer's job report + receipt is built by the office one section at a time, lives on the job, and is sent (and re-sent) with a button

**Context.** D-072.7 promised "customers get receipt + photo job report". Tim's brief: not all the photos by default — the office picks each photo and writes what it shows, section by section, in a Create Report popup; the Summary of work and the customer / job details merge in as the header; it is editable after it is finished and even after it is sent, and can be sent again; it goes out with a button like the estimate and the invoice; report and receipt are one document.

**Decision.**
1. **The report is a document ABOUT the job, stored ON the job** — `Report_Sections__c` (JSON `{ version, sections: [{ id, photoKey, caption }], receipt, intro }`), `Report_Public_Token__c` (+ expiry), `Report_Updated_At__c`, `Report_Sent_At__c`, `Report_Sent_Count__c`, `Report_PDF_S3_Key__c`. No report object, no version log: the send count and stamps are the history, every send's PDF stays in the job's Files (`job-report-{n}.pdf`), and the hosted link is the document of record. Rejected: a `Sundial_Job_Report__c` object (a second record to open for one page the customer keeps) and a Supabase-only table (the token lookup and the PDF key belong with the job, and a new tenant's Salesforce should carry its reports).
2. **One model, two painters, same as the estimate and the invoice.** `lib/job-report-document.js` decides what is shown — header (customer / job, `Customer_Summary__c`), the office's opening line, the sections (photo + caption, labelled by the visit), the receipt — and `renderJobReportDocument` (HTML) and `lib/job-report-pdf.js` (pdf-lib, photos embedded from their bytes) only draw it. **The receipt is `buildInvoiceModel`'s own rows and totals**, never a second set of numbers, and it is **left out when a partner pays** the job (the customer's copy never shows a partner's bill), when there is no live invoice, or when the office switches it off.
3. **A section's photo must be on the job** (`SUNDIAL/{jobId}/photos/…`, checked against the folder listing on save), captions capped, at most 60 sections. Text-only sections are allowed. The office edits after sending; the card flags "edited since sent".
4. **Send = render + store + email + optional text, best-effort past the write.** `POST /service/jobs/{id}/report/send` (action `service.estimate.send`) issues the token once (renewed for a year on every send), renders the PDF of this send, attaches it when under 8 MB (SES) and links it otherwise, emails the customer (or `to`) the hosted link, and texts the link through `lib/sms-send.js` when asked — the text lands on the job's conversation like every other. Each failure is reported in `deliveryDetail`, none loses the sections. The customer page is `GET /public/reports/{token}` on the public Lambda (read-only; 404 / 410 like the estimate).

**Consequences.** Nothing is sent automatically — the job page nudges when a job is Paid / Closed / Awaiting Office Review with no report sent. The generator now emits `service-delta-YYYY-MM-DD/` packages for fields tagged `added=` (this one carries the report fields, the two notes fields and `Customer_Type__c`). `sundial-service-estimate` gains S3 `getObject` / `listFiles` and the SMS sender; `sundial-service-public` gains `listFiles`. `EVENTS.JOB_REPORT_SENT`. Follow-ups: a "report not sent" marker on the Service jobs list; a customer-visible flag per photo was NOT built — the report's picks are the flag.

## D-073: Service Club memberships — Stripe subscriptions sold from Sundial's own pages, mirrored to Salesforce, with plan discounts on estimates (2026-09-18)

**Context.** Harmon pulled the Service Club forward on 9/1 (it is why they bought Housecall Pro, and HCP never delivered it). solarserviceclub.com today is a Replit landing page whose "Join Now" is a form that ends in "we'll email you an invoice" — a human step for every member. Stripe is now on (D-072 amendment 8), so the club can sell itself: pick a plan, pay on Stripe's hosted page, be a member. The catalog was read off the live site on 2026-09-18: Monitor $8.99/mo or $99.99/yr (monitoring, remote troubleshooting, priority scheduling, $50 off inspections, 10% off repair work, 5% off add-on equipment); Maintain $19.99/mo or $219.99/yr (+ annual tune-up); Clean $39.99/mo or $439.99/yr (+ annual panel cleaning); Protect $39.99/mo or $439.99/yr, "coming soon". ~24 existing members are on hand billing; Ben moves them.

**Decision.**

1. **Two new objects, tenant-scoped like everything else.** `Sundial_Service_Plan__c` is the catalog — one row per thing sold online (`Kind__c` Subscription or One-time), with the prices, the Stripe product/price ids, `Availability__c` (Available / Coming Soon / Hidden / Retired — "coming soon" is a real state, not a hack), the feature list, and the **member discount** in the estimate's own vocabulary (`Discount_Scope__c` Labor / Material / Both, `Discount_Type__c`, `Discount_Value__c`). `Sundial_Membership__c` (MEM-#) is one row per customer × plan × subscription: status (Pending / Active / Past Due / Cancelled / Expired), interval, price, the Stripe subscription id (the idempotency key for every subscription event), period end, cancellation stamps, lifetime revenue, the SolarFacts hand-off status, and the customer snapshot. `Sundial_Customer__c.Active_Membership__c` is a Lambda-maintained pointer to the one live row — the customer hub, the estimate and the job read membership through it in one hop. Generated by `scripts/gen-service-club.py` into `salesforce/service-club/` (same rule as D-072: edit the spec, never the outputs).
2. **The catalog is data, the Stripe side is derived from it.** `scripts/seed-service-club.mjs --tenant harmon --apply` upserts the plan rows and creates the Stripe Products and Prices from them, writing the ids back; a price change is a new Stripe Price and an archived old one (Stripe prices are immutable, so are ours). Nothing about Harmon's plans is in Lambda code.
3. **Everything public lives in the estimate Lambda under `/public/club/{tenant}/…`**, no login, the tenant slug in the URL exactly like the Stripe webhook: `GET plans`, `POST join` (customer find-or-create with the D-072 duplicate guard — a public visitor whose email or phone already matches is that customer, silently, never a 409 — then a **Pending** membership row, then a Stripe Checkout Session in `subscription` mode), `POST truck-roll` (an approved estimate + a job with the plan's one-time price, paid through the existing `deposit` Checkout so it lands through the existing webhook untouched), `POST request` ("I don't know what I need — call me": a job in `Needs_Intake_Review__c` + a team email), `POST manage` (emails the member a Stripe **customer-portal** link — card update, receipts, cancel — without ever returning it to the browser), `GET joined` (what the success page may show: plan, interval, status; nothing else). It lives in the estimate Lambda, not `sundial-service-public`, because the join and the truck roll reuse its customer, estimate and job creation code rather than copying it.
4. **Membership follows Stripe, through the same webhook.** `POST /webhooks/stripe/{tenant}` now also accepts `checkout.session.completed` in `subscription` mode (→ Active, subscription id stamped, `Active_Membership__c` set), `customer.subscription.updated` / `.deleted` (status, period end, cancel-at-period-end, Cancelled → pointer cleared), `invoice.paid` / `invoice.payment_failed` on a subscription (Last payment / Lifetime revenue, Past Due). Same signature gate, same ledger, same look-before-you-write on the subscription id. A subscription event that names no membership we know is `ignored` in the ledger, never a create — a membership is only ever born from our own join.
5. **The office manages memberships from Sundial, money moves only in Stripe.** `GET /service/club/memberships`, `GET /service/club/report`, `GET /service/club/customers/{id}`, `POST /service/club/memberships` (record a member and get a join link — Ben's migration: the customer enters their own card, the office never types one), `POST …/{id}/cancel` (cancel now or at period end, through Stripe, then the webhook confirms), `POST …/{id}/solarfacts` (resend), `GET/PATCH /service/club/plans`. Actions `service.club.read` and `service.club.write` (tenant scope) in `lib/access.js`.
6. **SolarFacts is a hook, stamped, retriable.** On activation Sundial POSTs the member (name, email, phone, address, plan, interval, membership number) to the tenant's Zapier catch-hook; on the end of a subscription it POSTs the same shape with `event: member.cancelled` to the cancel hook (or the same hook with the event name). Both URLs and the team's notification address are per tenant in Secrets Manager `sundial/service-club` (`tenants[slug]: { solarFactsHookUrl, solarFactsCancelHookUrl?, teamEmail }`) — a catch-hook URL is a credential. The result is stamped on the membership (`SolarFacts_Status__c`, last sent, last error); a failure never fails the webhook, and the office has a resend.
7. **Member discounts are applied at estimate creation and on demand, in the estimate's existing discount fields.** A new estimate for a customer with an Active membership gets the plan's discount (`Discount_Source__c = Service Plan`, `Membership__c` pointing at the row); the office can change or clear it like any discount, and `POST /service/estimates/{id}/apply-plan-discount` puts it back. One discount per plan, in the scope/type/value the estimate already computes — the site's three-way "$50 off inspections / 10% off repair / 5% off equipment" is expressed as the plan's one discount plus the office's judgment on the rest; a per-kind discount table is a later amendment if Harmon wants it exact.

**Consequences.** Team notification is email (SES, `lib/email.js`) to the tenant's club address on join, payment failure, cancellation and every "call me". The plans report is computed live (counts by plan × status, MRR as monthly-equivalent of Active rows, revenue from the Stripe ledger's `invoice.paid` rows). Stripe's customer portal must be enabled in the dashboard once (Tim). The Protect plan ships as Coming Soon and is switched on by editing the row. `Sundial_Service_Plan__c` and `Sundial_Membership__c` join the sf-query / cache-sync registries (`serviceplan`, `membership`, parent filter `Sundial_Customer__c`) so the portal lists them like any object. Never a card number, a publishable key or Stripe.js in the browser (amendment 8 stands).

### D-073 amendment 1 (2026-09-18, same day): SolarFax is their API, not a Zapier hook — invite on activation, disconnect at the end

**Context.** D-073.6 assumed SolarFacts (Solar Data Pros, "SolarFax") would take a Zapier catch-hook because that was the 9/1 plan. They have a real REST API (solardatapros.crunch.help → System Guides → API Docs): `POST /api/v1/users`, authenticated by two headers, `Api-Key` and `Access-Token`, which creates-or-updates a user + account (name, email, phone, address), can enable login and emails, can send a named email template, and takes a `disconnect` flag described as "full disconnect and data deletion for utility and solar". Harmon's requirement, in Tim's words: when a customer successfully pays, the connect-invite email goes to the customer; when the subscription ends (lapses or cancels), the customer is disconnected.

**Decision.**
1. **`lib/solarfacts.js` is the only place SolarFax is spoken to.** Two calls: `invite(member)` — `POST users` with `user { firstName, lastName, email, enableAccess: "1", enableEmails: "1" }`, `account { phone, addressOne, city, state, zip, isLead: "0" }`, `newOnly: "0"` (a member SolarFax already knows is updated, never refused), `sendEmailTemplate: { Name }` = the tenant's white-labelled connect template — **SolarFax sends the invite email, Sundial never does**; `disconnect(member)` — the same endpoint with `disconnect: "1"`. `fetch` is injectable; the key and token never reach a log line.
2. **Activation invites, the end disconnects — and only the end.** `checkout.session.completed` (Active) → invite; `customer.subscription.deleted` (Cancelled) → disconnect. **Past Due is not the end**: a failed renewal keeps the member connected until Stripe's retry schedule gives up and cancels the subscription (dashboard setting, runbook step 4), and that cancellation is the lapse. Rejected: disconnecting on the first failed payment — one declined card would delete a paying customer's monitoring history.
3. **Config per tenant in `sundial/service-club`:** `tenants[slug].solarFacts = { apiKey, accessToken, inviteTemplate, baseUrl?, test? }` — credentials, so Secrets Manager only. The Zapier shape (`solarFactsHookUrl` / `solarFactsCancelHookUrl`) stays as the fallback when `solarFacts` is absent; neither → `Not Applicable`. `test: true` sends SolarFax's own `test: "1"` (validate, do nothing) for dry runs.
4. **The membership remembers SolarFax's ids** (`SolarFacts_Account_Id__c`, `SolarFacts_User_Id__c`, from the API's `account_id` / `user_id`) next to the existing status / last sent / last error stamps. Everything else in D-073.6 stands: a refusal (`success: false`) or an outage is stamped `Failed` with SolarFax's message, named in the team email, never fails the webhook, and the office's Resend retries it.

**Consequences.** Tim asks SolarFax support for the Api-Key, the Access-Token, the invite template's name, and confirmation that `disconnect` is the right end-of-membership call (if they would rather stop monitoring without deleting, `disconnectBody` changes in one place). `salesforce/service-club/` gains the two id fields (redeploy); `sql/2026-09-18_service_club.sql` gains the two cache columns. `lib/solarfacts.test.js` covers the bodies, headers and error shapes; the estimate suite walks join → invite → delete → disconnect → refusal → resend against a fake SolarFax.


## D-074: Notifications — one notifier, a bell, browser pop-ups and Web Push, for the two audiences that need to be told (2026-09-21)

**Context.** Until now the only thing Sundial pushed to a person was an @-mention email (D-056), and the only thing the team heard about the club was an email to a shared address (D-073). Everything else — a call moved onto a tech's board, a customer texting back, an estimate approved online, a deposit paid, a tech clocking in — was visible only to whoever happened to be looking at the right page. Housecall Pro notified techs of schedule changes on their phones and the office of tech activity; the service team expects that. Tim's scope (2026-09-19): techs get **schedule changes, office @-mentions, customer text replies, day-before + one-hour reminders**; the office gets **tech activity** (clock in / complete / no-show / a call unstarted 30 minutes past its window), **money + approvals** (approved / declined online, deposit / invoice paid, payment failed, club join / cancel), **customer messages** (an inbound text, a website call-me) and **@-mentions**; the office style is **a bell in the header + browser notifications + push, opt-in in Settings**.

**Decision.**

1. **One module, three deliveries.** `lib/notify.js` `createNotifier()` is the only way a Lambda tells a person something: it inserts a row in `sundial_notifications` (the bell's list), broadcasts it on the Realtime channel `user:{profile_id}:notify` (an open tab rings at once), and Web-Pushes it to every device in `sundial_push_subscriptions` for that person. `toUsers({ userSfIds })` for techs and assignees, `toOffice()` for everyone in the tenant with `access_scope = 'tenant'` (minus the actor), `toProfile()` when the caller already holds the auth uuid (mentions). **Best-effort, always**: a notification rides on a write that has already succeeded, so nothing in the module throws to its caller — every entry point is guarded.
2. **The recipient key is the Supabase auth uuid** (`profiles.id`), never a `Sundial_User__c` id — it is what the browser's RLS compares against, so the read policy is the plainest possible `profile_id = auth.uid()` and the browser reads its own rows straight from Supabase (like comments and preferences), marking `read_at` as the one column it may update. The Lambdas translate Salesforce ids through `profiles.sundial_user_id`, which sundial-auth-proxy stamps on every `/auth/me`. A tech who has never signed in has no profile and gets nothing — there is no device to reach anyway.
3. **Dedupe is the unique `(profile_id, dedupe_key)`.** Every emitter names the event it is reporting (`money:paid:{pi}`, `schedule:moved:{call}:{tech}:{start}`, `late:{call}`, `mention:{id}`, `sms:{sid}`): a replayed webhook, a five-minute sweep that re-finds the same call, a tech who taps twice, a pg_net redelivery — the second insert is a no-op and rings nobody. A genuine second move carries a new start and rings again.
4. **Each person switches categories off, never on.** `user_preferences.notify_prefs` jsonb (`{ money: false }`); a missing key is ON — the comment-alert convention, so nobody has to opt in. `browser: false` silences native pop-ups while a tab is open. The `mention` bell is independent of the `comment_email_alerts` email switch: email off is not bell off.
5. **Push is `web-push` + VAPID, per device, through `sundial-notify`.** The pair lives in Secrets Manager `sundial/push` (`publicKey`, `privateKey`, `subject`); the private key never leaves the Lambda, the public key is what the browser subscribes with (`GET /notify/config`). `POST /notify/subscriptions` stores the browser's endpoint + keys **stamped with the verified JWT subject, never request input**; `DELETE` removes only the caller's own row; `POST /notify/test` rings the caller. A 404 / 410 from the push service deletes the subscription; anything else is stamped on the row and retried next time. The browser never reads `sundial_push_subscriptions` (no policy at all). Action `notify.self` — every signed-in scope but `none`, because it only ever touches the caller's own rows.
6. **Time-based events are a sweep, not a Flow.** `sundial-notify` is also invoked by EventBridge every 5 minutes (`sundial-notify-sweep`): one-hour reminders (`Scheduled`, starting within 65 minutes → the tech), the day-before digest (once per tech per local day at `REMINDER_HOUR`, default 5 pm, listing tomorrow's calls), and the late call (`Scheduled` 30+ minutes past its start with no tap → the office). One cross-tenant query, routed per `Client__c`, every row tenant-stamped.
7. **The emitters live where the event happens.** Dispatch board: `createCall` / `patchCall` / `cancelCall` → the tech (`scheduled`, `moved`, `reassigned_away`, `cancelled`; an Unscheduled call tells nobody until it is on the board); the tech's status taps → the office (`clock_in`, `complete`, `no_show` — **"on my way" is deliberately not an office alert**: seven techs a morning is noise). `sundial-sms` inbound → the office, and the tech on that job today (live, or scheduled for today's local date). `sundial-service-public` accept / decline → the office. `stripe.js` intent succeeded / failed → the office. `club.js` `notifyTeam()` now rings the bell before it emails (join, ended, past due, cancellation scheduled, online booking, call-me). `sundial-comment-notify` → the mentioned person, after every existing guard (self-mention, tenant, visibility) and regardless of the email switch.
8. **One service worker, two shells.** `public/sw.js` gains `push` / `notificationclick` (opens the portal path, focusing an open Sundial window; pushes and open-tab pop-ups collapse by `tag` = the row id). The tech app registers it on every load as before; the office registers it only when someone turns push on. The offline shell fallback is narrowed to `/tech/*` navigations — an office page that fails to load must fail visibly, never open the tech app. iPhone push works only from the Home-Screen app; the UI says so instead of failing.

**Consequences.** `sql/sundial_notifications.sql` (two tables + the column; the dedupe index is deliberately non-partial so `ON CONFLICT` can name it). New Lambda `sundial-notify` (console-created, `deploy.ps1`), `scripts/wire-notify-routes.ps1` (routes + the EventBridge rule), `web-push` in `package.json`. Portal: `NotificationsProvider`, the header bell, Settings → Notifications (push on this device, a test, the audience's four switches, browser pop-ups), the tech app's push banner + More → Notifications. Redeploy: auth-proxy (`lib/access.js`), board, sms, estimate, public, comment-notify, notify. Rejected: Supabase Realtime *postgres_changes* on the table (a replication slot and per-row RLS on the socket for something the Lambda already knows it wrote); SMS to techs (they have the app); Salesforce Flows for the reminders (no way to reach a browser from there). Later: a per-tech quiet-hours window; digest emails for people who never open the portal.

## D-075: The Customer in the Service module — one record, a second view, its own pipeline field (2026-09-23)

**Context.** Harmon's service office needs the Customer as the **pre-estimate record**: every call, email or web contact — a new lead or an existing solar customer with a question — lands there, stays a Lead until an estimate is warranted, gets assigned to a person in any department, and is tracked as open / unresolved by stage, assignee and follow-up date. Some of it is answered on the phone, referred to the manufacturer or the finance partner, or handed to another department, and never becomes a job. The Sales module's Customer page is the solar pipeline (adders, budget, commissions, appointment, proposal, contract — 300+ fields) and is exactly right for Sales; it is unusable for a service call. Tim: keep Sales as-is; give Service its own Customers tab, list, board and a smaller detail page; reuse existing fields wherever one exists; Files and Comments must be the hub's, because the solar install's documents and notes are where service insight comes from; Tim's Flows handle the follow-up reminders.

**Decision.**

1. **One record, two views.** The Service page (`/service/customers/{id}`) reads and writes the SAME `Sundial_Customer__c` through the same `GET /sf/customer/{id}?full=true` and `PATCH /sf/customer/{id}`, the same edit engine, field component, access helper, `FilesPanel` (`SUNDIAL/{customerId}/`) and `CommentsPanel` (`record_object = 'customer'`). Nothing is copied anywhere. The layout is a hand-written config (`harmon-crm/src/config/service-customer-config.ts`, schema `docs/service-customer-layout.md`): Request · Contact · Property · System & Accounts · Files, with the installed-system facts read-only (they are the solar project's).
2. **The Service pipeline is its own field, `Service_Stage__c`** (New → Contact Attempt Made → In Progress → Waiting on Customer → Waiting on Other Department → Estimate Created → Resolved → Closed), with `Service_Request_Type__c`, `Service_Resolution__c` and `Service_Resolved_Date__c` — four new fields (`salesforce/service-customer-2026-09-23/`). `Status__c` stays the hub's lifecycle and `Stage__c` stays the solar sales pipeline: reusing `Stage__c` would have put Service stages into every Sales dropdown and meant editing the Status→Stage dependency matrix. Everything else on the page is an existing field (`Description__c` = the request, `Outreach_Notes__c` = the contact log, `Assigned_To__c`, the follow-up dates).
3. **The Service list is the tagged set.** `GET /sf/customer?field=Customer_Type__c&value=Service&op=includes` — a new `op=includes` on the list read: a multi-select INCLUDES on the cache (`col.eq | like "v;*" | like "*;v" | like "*;v;*"`) and on the live SOQL path alike; the shadow count treats it as uncomparable. The search reaches the whole hub; an untagged customer opened in the Service view gets **Add to Service**.
4. **`POST /service/customers`** (estimate Lambda, `service.estimate.write`): the popups' select-or-create (`resolveCustomer` — duplicate guard, the Service tag on both department fields) followed by `Service_Stage__c = New` (never overwriting a stage already set) and the request the office typed (type, description, assignee + assigned date, next follow-up, lead source). Every Service field is describe-guarded so an org without the package still gets its customer, with a warning. Create Estimate / Create Job on the page are the existing popup with the customer pre-picked.
5. **Assigned To is a name, never an id.** The record carries the `Sundial_User__c` id (`assigned_to_sf_id` in the cache); the browser joins it to the tenant's user list. No formula field was added for it — a second Flow-visible field for the same fact was not worth a package member.

**Consequences.** `sql/2026-09-23_service_customer.sql` (ten customer-cache columns + two indexes) and one full customer resync. Routes: `POST /service/customers` (`scripts/wire-service-customers-route.ps1`). Portal: `ServiceCustomersPage` (table / board, filters: stage — default open —, assignee incl. "mine" / unassigned, status, request type; New Customer / New Estimate / New Job), `ServiceCustomerDetailPage`, `NewServiceCustomerModal`, `src/lib/service-customers.ts`; the customer's related bar gains **Estimates**. Not done yet: drag-to-stage on the board; the Sales side's New Customer popup keeps its own form. Tim's Flows: reminders off `Next_Follow_Up_Date__c` / `Last_Contact_Date__c` / `Assigned_To__c`, and (optionally) `Service_Resolved_Date__c` when the stage reaches Resolved / Closed.
