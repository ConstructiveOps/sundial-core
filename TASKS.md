...
## Phase 2 — data model v2 (D-072) — Stage 1 REWORKED to seven objects (2026-09-09), ready to deploy

Tim's post-9/9 structure is in `docs/service-data-model.md` + D-072. Tim confirmed 9/9: Stage 1 was never deployed; `Sundial_Customer__c` has ~100 spare fields (so `Stripe_Customer_Id__c` goes there). The rework below replaces the 2026-09-02 build in place.

- [x] **`scripts/gen-service-objects.py` — ONE spec generates everything** (objects, permission set, package.xml, 7 SQL files, 3 workbooks, `spec.json`). 7 objects / 201 fields / 190 FLS grants. Regenerate rather than hand-edit (`python scripts/gen-service-objects.py`, needs openpyxl).
- [x] **`salesforce/service-objects/` v2** — `Sundial_Estimate__c` (55 fields), `Sundial_Service_Job__c` (42; was `Sundial_Service__c`, no estimate states, `Estimate__c` required, Bill-To job-level), `Sundial_Service_Call__c` (27; was `_Visit__c`, no Bill-To override), `Sundial_Price_Book_Item__c` (19; versioned, no delete), `Sundial_Service_Line__c` (20; re-parented to the estimate, snapshot fields), `Sundial_Service_Invoice__c` (22; frozen amounts, no Stripe fields), `Sundial_Service_Payment__c` (16; unique `Stripe_Payment_Intent_Id__c`) + `Sundial_Customer__c.Stripe_Customer_Id__c` as a single CustomField member. XML well-formed. README rewritten (expect **9/9** on Check Only).
- [x] **7 cache tables** `sql/sundial_{estimate,service_job,service_call,price_book_item,service_line,service_invoice,service_payment}_cache.sql` — **applied cleanly to a scratch Postgres 16** (the comma-after-comment bug from the first pass is fixed in the generator, not by hand).
- [x] **Registries** — keys `estimate / job / servicecall / pricebookitem / serviceline / serviceinvoice / servicepayment` in `sundial-sf-query` (OBJECT_ALLOWLIST, CREATED_DATE_SOURCE, PARENT_FILTER: estimate/job→customer, serviceline→estimate, servicecall/serviceinvoice/servicepayment→job, pricebookitem none; SEARCH_FIELDS for job/estimate/pricebookitem/serviceinvoice/servicepayment), `sundial-sf-update`, `sundial-cache-sync`, `lib/access.js` (all `salesScopes: false`). Old keys `service`/`visit` are gone and now fail closed (pinned in the test).
- [x] **`lib/access.test.js`** matrix = 7 service columns × 6 levels — **162/162 green** (run in the cloud with stubbed `lib/secrets.js`/`lib/supabase.js`; expect the same on Windows).
- [x] `scripts/verify-service-schema.mjs` — understands the field-only `Sundial_Customer__c` addition and the three commercial lookups; `Date` type added.
- [x] Workbooks regenerated: `docs/Sundial_Estimate_Fields_by_Section.xlsx` (Estimate + Line + Price Book Item sheets), `docs/Sundial_Service_Job_Fields_by_Section.xlsx` (Job + Invoice + Payment), `docs/Sundial_Service_Call_Fields_by_Section.xlsx`. Yellow = system-maintained.
- [ ] **TIM: `git rm` the superseded files** — `salesforce/service-objects/objects/Sundial_Service__c.object`, `…/Sundial_Service_Visit__c.object`, `sql/sundial_service_cache.sql`, `sql/sundial_service_visit_cache.sql`, `docs/Sundial_Service_Fields_by_Section.xlsx`, `docs/Sundial_Service_Visit_Fields_by_Section.xlsx` (the VM cannot delete on your machine). Then `npm test` on Windows; commit on the feature branch.
- [ ] **TIM: deploy sequence** (README): `node scripts/verify-service-schema.mjs` → drop the 3 commercial lookups if flagged → zip contents → Workbench Check Only (**9/9**) → deploy → assign permission set → re-run verify → apply the 7 SQL files in Supabase → `.\deploy.ps1 sundial-sf-query sundial-sf-update sundial-cache-sync` → XFiles Pro for job / call / estimate.
- [ ] **Customer-in-the-popup (Tim, 9/9; data-model §3.1a):** New Estimate / New Job select an existing customer OR create one (first, last, address, email, phone) in the same request; `Requested_Project_Types__c` silently set to `Service` (new) or union-added (existing); soft duplicate guard (email / phone / street+zip → 409 with candidates, `confirmNew` to override); reuse the `POST /sf/customer` validation path. Same pattern reserved for Roofing / Commercial buttons later. **Verify script now checks the "Service" picklist value exists** — add it in Setup if it reports MISSING. **Get from Tim:** Stage/Status/Lead_Source defaults for a service-originated customer.
- [ ] **Next build (Claude):** `sundial-service-estimate` Lambda — line CRUD with item snapshot, totals math (data-model §6), send (version++, PDF, `Version_Log__c`, hosted-page token), accept/decline, *Create Job*, quick-create (estimate + job), price-book *Update* clone + one-active-per-code + reference-count edit lock; then portal detail configs from the workbooks (harmon-crm), then `sundial-service-board`.
