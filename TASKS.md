...
- [ ] **TIM: deploy the fixes** — (1) `npm test`; (2) `.\deploy.ps1 sundial-service-estimate` and `.\deploy.ps1 sundial-service-board` (the CORS fix + the line link + the 42-day window; nothing to wire); (3) `node scripts/seed-access-test-fixtures.mjs --users-only` to see the plan, then `node scripts/seed-access-test-fixtures.mjs --apply --users-only` (creates the two techs; `--show-passwords` prints the map from Secrets Manager); (4) harmon-crm: `npm run build`, commit, push `main`; (5) test: edit an estimate's tax rate — the toast should be gone and the totals update; on an ad-hoc line click *Save to price book* → Create → the line reads "Price book"; Dispatch → the board opens on the week with three ZZ tech rows once the seeder has run.
