...
- [ ] **Next (Claude):** Stripe SetupIntent / deposit on the accept step when keys arrive; SMS via Twilio (board notify + estimate send); portal detail configs from the three workbooks (harmon-crm); invoice issue + payments (`Sundial_Service_Invoice__c` / `_Payment__c` routes) + Stripe webhook worker; the customer photo report; the tech PWA.
