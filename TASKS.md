...
- [!] **Rep-form calls never write their ANALYSIS to Salesforce — decide how to close this.** All three live calls so far arrived with no `sf_record_id`, so the whole writeback is skipped: no status, no log line, none of `mismatched_items` / `follow_up_notes` / `call_summary`. The orphan-match sweep then appends only `rep-form call <id> matched, recording attached` — it attaches the audio and **backfills nothing**. So today that analysis exists only in the Zapier ledger. Options: (a) have the sweep re-fetch the call from Retell (`GET /v2/get-call/{call_id}` — we already hold the API key) and write the full result line, (b) have the Zap post the analysis fields to `/welcome-call/orphan-match`, or (c) accept the ledger as the system of record for rep-form calls. (a) is the most self-contained and needs nothing from Zapier beyond what it already sends.
...
- [!] **Wire AWS SES** (shared `lib/email.js` scaffolded 2026-07-30; `@aws-sdk/client-sesv2` added; NOT wired to any feature). Blockers/steps: (a) create + verify the sending domain identity — recommended `mail.constructiveoperations.com` — and add the DKIM CNAMEs + SPF/DMARC; ~~(b) request SES production access (out of sandbox)~~ **(b) DONE — AWS moved the account out of the SES sandbox 2026-08-03, effective immediately in us-west-1 (support case 178572585300376). Verified from the AWS notification, not from the console.**; (c) grant the Lambda role `ses:SendEmail` and set `EMAIL_FROM`/`SES_REGION` env vars on senders — **still open; this, not sandbox status, is what keeps Design Request notifications degrading to `email_not_configured`.** Consumers waiting on this: **Design Request → email the design manager** (now BUILT in `sundial-aurora-push`, D-047 — it degrades to `email.sent: false, reason: "email_not_configured"` until `EMAIL_FROM` + `DESIGN_REQUEST_NOTIFY_TO` are set) and **@-mention alerts**. NOTE: **Supabase Auth invite/reset no longer waits on this SDK path** — it goes through Supabase Custom SMTP → SES instead (D-046, `docs/integrations/auth-email-ses.md`), which is independent of `lib/email.js`.
