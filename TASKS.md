...
- [x] **GATE: `CREATE_GATE = { enabled: false }` — a repo constant, NOT an env var.** An env var can be flipped in the AWS console with no commit and no review, and we were burned once by a load-bearing untracked dashboard setting. A test asserts the committed value is `false`. While closed the behaviour is **exactly** pre-D20 (loud abort before any PUT), also pinned by a test — "disabled" has to mean unchanged, not quietly different.
- [x] Tests: **56** (was 38). Covers all three branches, gate open and closed, and four ways a create can go wrong — rejected, 200-but-nothing-created, 200-but-duplicated, and wrong AccountGroup.
- [!] **TIM: run the sandbox hand-proof** — `docs/integrations/acumatica-referral-line-create-runbook.md` against BizRun **R269999**. Five REST calls: mint token, create, re-read for the guid + AccountGroup/Type, update by guid, re-read to prove no duplicate. Paste the results. **The gate stays closed until then.** If step 2 returns "Inventory item REFERRAL not found", that is the real answer and Harmon has to create the item first; if PUT-without-id is not an insert on this entity, D20 is not implementable as designed and needs rethinking before anything else.
