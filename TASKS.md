...
- [ ] **DEPLOY** — create the function in the console (Node.js 22.x, `index.handler`, `sundial-lambda-execution-role`, **60 s / 512 MB** — 60 s is the floor now: ~10 s Zapier retries + ~20 s recording download + SF), then `.\deploy.ps1 sundial-welcome-call`, then `.\scripts\wire-welcome-call-routes.ps1`.
...
- [ ] **Consider widening `Energy_Rate__c` to 4 decimal places** (Tim, Setup). It is Currency **scale 2**, so a `$0.1425/kWh` rate stores as `0.14` — ~1.8% off on a number that feeds customer-facing math. One field change, worth doing before lease/PPA volume arrives.
