...
- [ ] **Board follow-ups:** FullCalendar Premium (edge-drag resize, week timeline) when the license lands — same API; the tech PWA (own calls, clock in/out with GPS, photos) with its own narrower actions; travel-time hints + tech map (design §8).
- [ ] **Next (Claude):** Stripe SetupIntent / deposit on the accept step when keys arrive; board notify + estimate send by text (Twilio is in); portal detail configs from the three workbooks (harmon-crm); Stripe webhook worker; the customer photo report; the tech PWA.
