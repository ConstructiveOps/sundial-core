...
Harmon Phase 1 punchlist: see ../harmon-crm/docs/HARMON_PHASE1_PUNCHLIST.md — BE-owned items: G2, E1.
...
- [x] **Aurora "Submit Design Request" endpoint:** `POST /projects/{solarId}/design-request/submit` (server-side Solar→Customer resolution) wired + deployed; email seam left for post-SES. Frontend button (Design Request Form tab) is a separate harmon-crm task.
- [ ] **Design Request Form fields (new SF fields on Sundial_Solar__c):** Workbench package pending Tim's field-existence markup of `Fields_by_Section.xlsx` (picklists, multiselect Sales Type, datetime, text/number) + FLS on integration + user perm sets.
