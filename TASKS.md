...
- [x] **Estimate PDF + Files tab on the service objects (2026-09-11, D-072 amendment 3):** `lib/estimate-document.js` split into `buildEstimateModel()` + the HTML painter; new `lib/estimate-pdf.js` (pdf-lib) paints the same model. Every `/send` writes `SUNDIAL/{estimateId}/estimate-v{n}.pdf`, registers the metadata row, records `Version_Log__c[].pdfKey`, attaches the PDF to the customer email (`lib/email.js` gained `attachments` via `Content.Simple.Attachments` — still `ses:SendEmail`), returns `pdfKey`/`pdfUrl`; the public GET returns `pdfUrl` for the viewed version. `lib/file-access.js` allowlist + `lib/access.js` action rows for `estimate` / `job` / `servicecall` — and for `roofing` / `po`, which had never had rows (their Files tabs were 403 for everyone); a test now pins allowlist ↔ actions. Tests: estimate 27, public 5, pdf 3, access 164.
- [ ] **TIM: deploy this increment** — (0) `.\deploy.ps1 sundial-auth-proxy` **first** — it bakes in its own copy of `lib/access.js`, and the one deployed in August has no service keys, so `/auth/me` reports no `job` module and the portal hides the Service tab (2026-09-11 — "the tab disappeared"). **Rule: any change to `lib/access.js` means redeploying `sundial-auth-proxy` too**, because it is the Lambda that tells the portal what to show; (1) `npm install` (pulls `pdf-lib`; commit the updated `package-lock.json`); (2) `npm test`; (3) `.\deploy.ps1` for **five** functions: `sundial-service-estimate`, `sundial-service-public`, `sundial-list-files`, `sundial-upload-file`, `sundial-delete-file` (the last three only pick up the new allowlist/action rows — no new routes, nothing to wire); optionally `sundial-list-related-files` for the same reason; (4) harmon-crm: `npm run build`, commit, push `main`; (5) test with the ZZ customer: Send → the email arrives with `EST-xxxx-v1.pdf` attached, the estimate page's Files panel shows `estimate-v1.pdf`, the customer page shows *Download PDF*; open a Roofing project's Files tab — it should list instead of erroring.
- [x] **Service calls + the dispatch board (2026-09-11, D-072 amendment 4):** new Lambda `lambdas/sundial-service-board` — `GET /service/board` (techs + calls in a window + the unscheduled tray, always fresh), `GET/POST /service/jobs/{id}/calls`, `PATCH /service/calls/{id}` (move / reassign / progress / notes, `baseModstamp` → 409 `CALL_CONFLICT`, started calls can't move), `POST /service/calls/{id}/cancel` (reason required). Job status follows the calls in `settleJobStatus`. Activity rows (`service_call_created | updated | cancelled` + `job_updated`), cache stale flags, Realtime broadcast on `tenant:{id}:sundial_service:list`, per-action customer email (SES) with the window. `lib/access.js` actions `service.board.read`, `service.call.write`. `scripts/wire-service-board-routes.ps1`. Tests 6/6. Portal: `/service/dispatch` (day board: tech columns, tray drag-drop, drag to move, click for progress / reschedule / cancel, Realtime + focus refetch), Dispatch tab, job page **Service calls** card; 3 render tests, 105/105.
- [ ] **TIM: stand up the board** — (1) Lambda console → Create function **`sundial-service-board`** (same runtime / architecture / role / timeout / memory as `sundial-sf-update`); env vars `SERVICE_BRAND_NAME = Harmon Electric`, `EMAIL_FROM`, `EMAIL_REPLY_TO`, `SES_REGION`, `EMAIL_CONFIG_SET` (same values as the estimate Lambda); `SERVICE_TIMEZONE` only if not Phoenix; (2) `npm test`; (3) `.\deploy.ps1 sundial-service-board` **and** `.\deploy.ps1 sundial-auth-proxy` (two new action rows — the proxy bakes in `lib/access.js`); (4) `.\scripts\wire-service-board-routes.ps1`; (5) Supabase → Database → Replication is NOT needed (broadcast, not postgres_changes) — but the Realtime broadcast HTTP endpoint must be enabled on the project (it is, `lib/realtime.js` already uses it for Welcome Call); (6) harmon-crm `npm run build`, commit, push `main`; (7) test with the ZZ customer: create a job → Service → Dispatch → drag it onto a tech → Schedule → the block appears, the job shows Scheduled, the email arrives; drag the block to another tech; click it → In Progress → Complete → the job shows Awaiting Office Review. Open two browser tabs on the board and move a block in one — the other refreshes.
- [x] **First live-test fixes (2026-09-11):** CORS `Allow-Methods` gains PATCH/PUT in `lib/http.js` (the "Save failed." toast on tax / markup / discount edits — the preflight was refused); `PATCH …/lines/{lineId} { priceBookItemId }` links an ad-hoc line to an active item (`linkLineToItem`); board window cap 31 → 42 days for the month grid; `zz-tech-2` / `zz-tech-3` fixtures (Technician + Service department). Portal: price-book typeahead "Create … as a new item", ad-hoc line "Save to price book" / "Add & save to book" (the item popup prefilled from the line, then the line is linked), dispatch board Day / **Week (default)** / Month with drag-drop in every view + tech filter. Tests: estimate 28, board 6, portal 109.
- [x] ~~TIM: deploy the fixes~~ — folded into the deploy step below (same Lambdas).
- [x] **Invoices + payments (2026-09-11, D-072 amendment 5):** `invoice.js` inside `sundial-service-estimate` — issue (estimate lines frozen, `-2` reissue, deposits back-filled, estimate locked, job Invoiced/Paid, PDF in the job folder), record payment (check / ACH / remittance / refund), send (fresh PDF, receipt once paid, partner address), void (reason, payments kept, estimate reopens, job Ready to Bill); `settleMoney()` owns Paid_Amount / statuses. Action `service.invoice.write`. Portal: job page Invoice card + Record-payment dialog + Invoices tab. Tests: estimate 31, portal 112.
- [x] **Street View on the job page (2026-09-11, the 9/9 ask):** `GET /service/jobs/{id}/street-view` (key in Secrets Manager `sundial/google-maps`, fetched once, cached in S3, remembered on the job); `StreetViewCard` on the job page.
- [ ] **TIM: deploy this increment** — (1) `npm test`; (2) `.\deploy.ps1 sundial-service-estimate` (CORS fix + line link + invoices + street view), `.\deploy.ps1 sundial-service-board` (CORS fix + 42-day window), `.\deploy.ps1 sundial-auth-proxy` (new action `service.invoice.write` — the proxy bakes in `lib/access.js`); (3) `.\scripts\wire-service-estimate-routes.ps1` (adds the invoice + street-view routes; idempotent; prompts before the prod deploy); (4) `node scripts/seed-access-test-fixtures.mjs --apply --users-only` (the two extra techs); (5) **Google key:** Google Cloud Console → a project → APIs & Services → enable **Street View Static API** → Credentials → Create API key → restrict it to that one API (no referrer restriction needed; it is server-side) → Secrets Manager → Store a new secret → *Other type*, key `apiKey` = the key, name **`sundial/google-maps`** (plaintext `{"apiKey":"…"}`) → confirm the estimate Lambda's role can read it (it reads `sundial/*` today); (6) harmon-crm: `npm run build`, commit, push `main`; (7) test with the ZZ customer: job → Invoice card → Issue → Record payment (check) → Paid; Void with a reason → Issue again → `SVC-xxxxx-2`; Invoices tab lists it; the Street view card shows the house (or "not set up yet" until step 5).
- [x] **Labor billing, "schedule later" calls, price-book filters, Communications + texting (2026-09-15, D-072 amendment 6):** HCP price book → `salesforce/pricebook-import/` (DataLoader upsert on `HCP_Id__c`); field delta `salesforce/service-delta-2026-09-15/` + `sql/2026-09-15_service_delta.sql`; `labor.js` (`GET/POST /service/jobs/{id}/labor`, `POST /service/labor/default-rate`); board `Unscheduled` calls (`unscheduled: true`, `unscheduledCalls` tray); item `jobType` / `serviceType`; invoice prints the job's `Customer_Summary__c`; new `lambdas/sundial-sms` (Twilio; `sql/sundial_sms_messages.sql`; `scripts/wire-sms-routes.ps1`; `docs/integrations/sms-twilio.md`); `service.sms.send`; comment-notify link map gains `job`. Portal: Labor billing card, Schedule-later + tray call cards, Price Book filters + item fields, Summary of work, collapsed Activity log, **Communications** panel (notes + texts + selected events). Tests: estimate 33, board 7, sms 13; portal 121.
...
- [x] **Time corrections on the board (2026-09-17):** `GET/POST /service/calls/{id}/clock` — the office edits / closes / removes / adds intervals with a reason; every change stamped onto the log, removed rows kept flagged, actuals + duration re-derived, `complete: true` finishes the call; the board's own status menu now moves the same clock. Portal: "Fix time" on the call dialog with History. Tests: board 17, portal 143.
- [ ] **TIM: deploy the time corrections** — (1) `npm test`; (2) `.\deploy.ps1 sundial-service-board`; (3) `.\scripts\wire-service-board-routes.ps1` (adds `/service/calls/{id}/clock`; prompts before the prod deploy); (4) harmon-crm: push `main`; (5) on the ZZ job: as `zz-tech-2` clock in on the phone and leave it; on the board open the call → Fix time → give it an end, a reason, tick Complete → Save; the call shows Complete with the minutes, the phone's visit page shows the clock idle, the job's Activity shows "Time corrected by the office".
- [x] **Addresses + related jobs + job photos/files + GPS (2026-09-18):** service address editable on the job (geocode + Street View reset) and the estimate (`address` patchable); Service Jobs (+ Roofing on) in the customer's related-records bar, empty groups hidden, Commercial noted for Phase 3; one photo folder per job read by the office (`GET/POST /service/jobs/{id}/photos[/confirm]`, `JobPhotosCard`) and the tech app (`GET /service/tech/jobs/{id}/photos`, read-only) + the job's Files in the tech app (`GET /service/tech/jobs/{id}/files`); GPS map links on Fix time rows. Tests: board 19, portal 154.
- [ ] **TIM: deploy 2026-09-18** — (1) `npm test`; (2) `.\deploy.ps1 sundial-service-estimate` and `.\deploy.ps1 sundial-service-board`; (3) `.\scripts\wire-service-tech-routes.ps1` (adds the five photo/file routes; prompts before the prod deploy); (4) harmon-crm: push `main`; (5) Tim's SF Flow: `Sundial_Service_Job__c.Address_at_Creation__c` ↔ `Sundial_Estimate__c.Address_at_Creation__c` both ways (guard against the loop: only write when the values differ); (6) on the ZZ job: change the address on the job page → the estimate shows it (Flow) and Street view re-fetches; add a photo from the job page → it appears under "Office" on the phone's job page; open the ZZ customer → "Service Jobs (n)" chips.
- [ ] **Tech app follow-ups:** per-tenant checklist templates (`Checklist_Template_Key__c`); customer-visible photo flag + the photo job report; materials used (price-book pick straight into the job's lines is there; a "used from truck stock" list is not); install-crew mode (Visit_Type Solar/Roofing with project context); a tech map on the board from the GPS fixes. (Push for a new / changed call landed 2026-09-21, D-074.)
...
- [x] **Stripe payments (2026-09-17, D-072 amendment 8):** Checkout on the hosted page (card on file / deposit / balance), `POST /webhooks/stripe/{tenant}` into `settleMoney`, off-session charge (`/charge` + `chargeCard` on issue), refunds mirrored, deferred deposits landed by Create Job, `sundial_stripe_events` ledger. `lib/stripe.js`. Tests: lib 4, public 7, estimate 37; portal 148. Runbook `docs/integrations/stripe.md`.
- [ ] **TIM: stand up Stripe (test mode first)** — follow `docs/integrations/stripe.md` §Setup: (1) restricted key + webhook endpoint (`…/prod/webhooks/stripe/harmon`, the four events) in the Stripe dashboard; (2) Secrets Manager `sundial/stripe` = `{ "tenants": { "harmon": { "secretKey", "webhookSecret" } } }`; (3) Supabase: `sql/sundial_stripe_events.sql`; (4) `.\deploy.ps1 sundial-service-estimate` and `.\deploy.ps1 sundial-service-public`, env `SERVICE_PUBLIC_BASE_URL` on the public Lambda; (5) `.\scripts\wire-service-estimate-routes.ps1` and `.\scripts\wire-service-public-routes.ps1`; (6) portal `main`; (7) the test-mode walkthrough on the ZZ customer with card 4242; then swap in live keys.
- [ ] **Invoice follow-ups:** receipt + photo job report email on payment (Stripe's own receipt is what the customer gets today); `Acumatica_Ref__c` / `Acumatica_Entered_At__c` hand-entry on the Invoices tab (Heather's bridge) and the weekly digest email; receipt + photo job report for customers (the "two send buttons"); partner terms (net days) as tenant config.
- [ ] **Get from Harmon for the board:** which users are techs (set Access Level = Technician, or Default Department = Service, on each — until then every active user is a column); the service day's hours (code: 6a–7p); default call length (code: 2 h); whether "notify the customer" should default on.
- [ ] **Board follow-ups:** FullCalendar Premium (edge-drag resize, week timeline) when the license lands — same API; travel-time hints + tech map (design §8) — the live last-location map of every tech on the board, from the clock's GPS fixes (the per-tap fixes already show on Fix time, 2026-09-18). ~~the tech PWA~~ → built (amendment 7).
- [ ] **Next (Claude):** Stripe SetupIntent / deposit on the accept step when keys arrive; board notify + estimate send by text (Twilio is in); portal detail configs from the three workbooks (harmon-crm); Stripe webhook worker; the customer photo report.
...
- [x] **Workstream S — the Service Club (2026-09-18, D-073):** `Sundial_Service_Plan__c` + `Sundial_Membership__c` (`salesforce/service-club/`, `scripts/gen-service-club.py`), `scripts/seed-service-club.mjs` (rows + Stripe products / prices), `club.js` in the estimate Lambda (public join / truck roll / call me / manage, the webhook's subscription branch, office memberships / report / plans, plan discounts on estimates), SolarFax through their API (`lib/solarfacts.js`, D-073 am. 1: invite on activation, disconnect when the subscription ends; Zapier hook as fallback) + team email per tenant in `sundial/service-club`, `service.club.read/write`. Portal: `/club/*` public pages, Service → Service Club, member chips. Runbook `docs/integrations/service-club.md`. Tests: estimate 44, solarfacts 4, portal 164.
- [ ] **TIM: stand up the Service Club** — the runbook's Setup, in order: (1) Salesforce `salesforce/service-club/` (Check Only 5/5 → deploy → assign `Sundial_Service_Club` to the integration user); (2) Supabase: `sql/sundial_service_plan_cache.sql`, `sql/sundial_membership_cache.sql`, `sql/2026-09-18_service_club.sql`; (3) `node scripts/seed-service-club.mjs --tenant harmon` then `--apply` (test keys); (4) Stripe dashboard: add the four subscription events to the webhook, widen the restricted key (Subscriptions, Products, Prices, Customer portal sessions), enable the Customer portal; (5) SolarFax support: Api-Key, Access-Token, the connect-invite template name, confirm `disconnect`; (6) Secrets Manager `sundial/service-club` (`solarFacts { apiKey, accessToken, inviteTemplate }` + team email — leave `solarFacts` out until the credentials arrive); (7) `npm test`; `.\deploy.ps1 sundial-service-estimate`, `sundial-auth-proxy`, `sundial-sf-query`, `sundial-cache-sync`; `.\scripts\wire-service-club-routes.ps1`; (8) harmon-crm `main`; (9) the runbook's test walk-through on the ZZ customer with card 4242; (10) point solarserviceclub.com's buttons at `/club/join?plan=…`. Steps 1–4 done 2026-09-18 (redeploy the SF package + re-run the SQL once for the SolarFax id fields).
- [x] **Six small fixes (2026-09-19):** office GPS strip per call, the house + remote diagnosis on the phone, `Customer_Type__c` (delta field, defaults, tech filter), the job's two notes fields on the desktop layout, call notes rolled onto the job at completion (D-072 am. 9).
- [x] **The customer's job report + receipt (2026-09-19, D-072 am. 10):** section builder on the job page, JSON on the job, one model / two painters (`lib/job-report-document.js`, `lib/job-report-pdf.js`), `report.js` get / save / preview / send (email + PDF + text), `/public/reports/{token}` + portal `/report/{token}`, delta package `service-delta-2026-09-19`.
- [ ] **TIM: job report go-live** (folds the 09-19 fixes' Salesforce step in): (1) deploy `salesforce/service-delta-2026-09-19/` (zip the folder's contents → Workbench → Check Only, **11/11** — 10 fields + the permission set — then deploy; it carries the report fields, the two notes fields and `Customer_Type__c`, and re-grants FLS); (2) Supabase: `sql/2026-09-19_customer_type_job_notes.sql` (now also the three report columns); (3) `.\deploy.ps1 sundial-service-estimate`, `sundial-service-public`, plus the 09-19 fixes' `sundial-service-board` / `-sf-update` / `-cache-sync` / `-sf-query`; (4) `.\scripts\wire-service-estimate-routes.ps1` and `.\scripts\wire-service-public-routes.ps1` (the report routes) and `.\scripts\wire-service-tech-routes.ps1` (the tech street-view route); (5) the estimate Lambda needs `s3:GetObject` on `sfsolproj/SUNDIAL/*` (it only had Put/Delete) — check the role; (6) harmon-crm `main`; (7) on the ZZ job: Create report → two sections → Preview → Send to your own email → open the link, the PDF, and the Files card.
- [ ] Job report follow-ups: drag-to-reorder sections; a photo's tech caption pre-filling the description. (The "report not sent" marker on the Service jobs list landed 2026-09-21.)
- [x] **Notifications (2026-09-21, D-074):** `lib/notify.js` (bell row + Realtime + Web Push, dedupe, per-person switches), `sundial-notify` (config / subscribe / unsubscribe / test + the EventBridge reminder sweep), emitters in board / tech / sms / public / stripe / club / comment-notify, `notify.self`, `sql/sundial_notifications.sql`, runbook `docs/integrations/push-notifications.md`. Portal: the header bell, Settings → Notifications, the tech app's banner, `sw.js` push.
- [ ] **TIM: notifications go-live** (`docs/integrations/push-notifications.md` → One-time setup): (1) Supabase: `sql/sundial_notifications.sql`; (2) `npm install` in sundial-core (web-push), then `node -e "console.log(JSON.stringify(require('web-push').generateVAPIDKeys()))"` → Secrets Manager `sundial/push` `{ publicKey, privateKey, subject: "mailto:…" }`; (3) create Lambda `sundial-notify` (like sundial-sms) → `.\deploy.ps1 sundial-notify`; (4) `.\scripts\wire-notify-routes.ps1` (routes + the `sundial-notify-sweep` EventBridge rule — needs events:PutRule / PutTargets); (5) `.\deploy.ps1 sundial-auth-proxy`, `sundial-service-board`, `sundial-sms`, `sundial-service-estimate`, `sundial-service-public`, `sundial-comment-notify`; (6) harmon-crm `main`; (7) bell → gear → Turn on → Send me a test; on a phone, Home-Screen app → banner → Turn on.
- [x] **Invites + password resets sent by Sundial (2026-09-22):** `lib/auth-email.js` (unspent `token_hash` link, SES), `sundial-user-admin` invites + `PATCH { resendInvite }` (re-points a re-created login), `sundial-auth-proxy` `POST /auth/forgot` (public, always 200, limited) + `scripts/wire-auth-forgot-route.ps1`; portal "Send invite / Resend link" on Manage Users, login page → `/auth/forgot` with the Supabase fallback. `sql/2026-09-21_user_delete_rules.sql` (comments SET NULL, mentions CASCADE) so a login can be deleted.
- [ ] **TIM: auth email go-live:** (1) Supabase SQL: `sql/2026-09-21_user_delete_rules.sql`; (2) Lambda env `EMAIL_FROM` (+ `EMAIL_REPLY_TO` / `EMAIL_CONFIG_SET` if the estimate Lambda has them) on **sundial-user-admin** and **sundial-auth-proxy**; (3) `.\deploy.ps1 sundial-user-admin`, `.\deploy.ps1 sundial-auth-proxy`; (4) `.\scripts\wire-auth-forgot-route.ps1`; (5) harmon-crm `main`; (6) Manage Users → Send invite to a ZZ address, hover the button in the email (must be `sundial.harmonelectric.net/reset-password?token_hash=…`), wait 10 min, click; login page → Forgot password → same check.
- [x] **PDF attachments fixed (2026-09-22):** SES attachments declared `ContentTransferEncoding: BASE64` (`lib/email.js`); the estimate / invoice / job-report PDFs open again after `.\deploy.ps1 sundial-service-estimate`.
- [x] **Tech app: the job from the call (2026-09-22):** job number link + "Open the job" on `/tech/calls/{id}`.
- [x] **Address lookup on customer create (2026-09-22, Harmon):** `sundial-service-estimate/address.js` (Google Places API (New), key server-side, per-entry session), `scripts/wire-address-lookup-routes.ps1`; portal `AddressLookup` in the New Estimate / New Job "New customer" form.
- [ ] **TIM: address lookup go-live:** (1) Google Cloud → the project behind the `sundial/google-maps` key → enable **Places API (New)** (and add it to the key's API restrictions if the key is restricted); (2) `.\deploy.ps1 sundial-service-estimate`; (3) `.\scripts\wire-address-lookup-routes.ps1`; (4) harmon-crm `main`; (5) Service → New Job → New customer → type "1 palm" in Street.
- [x] **The brand on every document (2026-09-24):** `lib/brand.js` (Secrets Manager `sundial/brand`), logo + identity lines + the terms / Service Club footer on the estimate, invoice and job report — page and PDF.
- [ ] **TIM: brand go-live:** (1) Secrets Manager → new secret **`sundial/brand`** (plaintext JSON — the block from this session's notes, with the real phone / email / ROC line); (2) `.\deploy.ps1 sundial-service-estimate`, `.\deploy.ps1 sundial-service-public`; (3) Preview an estimate on the ZZ customer: the logo at the top, the two links at the bottom; Send it to yourself and open the PDF.
- [x] **Dispatch board: invoice border + hover card (2026-09-23):** `invoiceStateFor` + the invoice lookup on `GET /service/board`; portal `CallHoverCard` / `callCardInfo.ts`. Deploy `sundial-service-board`.
- [x] **The Customer in the Service module (2026-09-23, D-075):** `docs/service-customer-layout.md`; `salesforce/service-customer-2026-09-23/` (4 fields + permission set); `sql/2026-09-23_service_customer.sql`; sf-query `?op=includes`; `POST /service/customers` + `scripts/wire-service-customers-route.ps1`; portal Service → Customers (list / board / page / New Customer).
- [ ] **TIM: Service customer go-live:** (1) Salesforce: zip the CONTENTS of `salesforce/service-customer-2026-09-23/` → Workbench → Check Only, **5/5**, deploy; (2) Supabase SQL: `sql/2026-09-23_service_customer.sql`; (3) `.\deploy.ps1 sundial-sf-query`, `.\deploy.ps1 sundial-service-estimate`, `.\deploy.ps1 sundial-cache-sync` (no code change, but it must see the new columns — a warm instance caches the column list; a redeploy restarts it); (4) `.\scripts\wire-service-customers-route.ps1`; (5) cache-sync full resync of customer: Lambda → Test → `{ "object": "customer", "mode": "full" }`; (6) harmon-crm `main`; (7) Service → Customers → New Customer with a ZZ name → the page → Edit → Assigned to → Save; search an existing ZZ solar customer → Add to Service → it appears on the board.
- [ ] Service customer follow-ups: drag a card between stage columns on the board; `Service_Resolution__c` / `Service_Resolved_Date__c` auto-filled by Create Estimate / Create Job (today the office sets them); the Sales New Customer popup could reuse `AddressLookup`; Tim's Flows for the follow-up reminders.
- [ ] Address lookup follow-ups: the Sales side's New Customer popup (`NewCustomerModal`) and the customer page's address edit could use the same `AddressLookup`; a territory bias in `sundial/google-maps` (`bias: { lat, lng, radiusMeters }`) once Harmon says where.
- [ ] Notification follow-ups: quiet hours per tech; a digest email for people who never open the portal; an "Unmatched texts" page so the office's `customer_message` for an unmatched text has somewhere better than `/service` to land.
- [ ] **TIM: 2026-09-19 fixes go-live:** (1) Salesforce: the three new fields already exist (you made them) — grant the integration user FLS by redeploying `salesforce/service-objects/` (whole package, Check Only first, 11/11) OR tick Read+Edit on `Sundial_Service_Objects` for `Sundial_Customer__c.Customer_Type__c`, `Sundial_Service_Job__c.Notes_for_Summary__c`, `Notes_From_Service_Calls__c`; (2) Supabase: `sql/2026-09-19_customer_type_job_notes.sql`; (3) `.\deploy.ps1 sundial-service-board`, `sundial-service-estimate`, `sundial-sf-update`, `sundial-cache-sync`, `sundial-sf-query`; (4) `.\scripts\wire-service-tech-routes.ps1` (adds `/service/tech/jobs/{id}/street-view`); (5) harmon-crm `main`.
- [x] **Tim's item 7 (2026-09-19)** — there was no item 7 (confirmed 2026-09-21).
- [ ] **Customer_Type__c for sales reps:** add the sheet row so the Sales-rep field manifest (`lib/field-manifest/customer.json`) lists it; a desktop Customers-list filter on it (the list API has no per-field filter yet).
- [ ] **TIM: Salesforce sharing (2026-09-18):** Setup → Sharing Settings → Edit → every `Sundial_*` object: Default Internal Access **Public Read/Write**, Default External Access **Private** → Save; wait for the recalculation; `node scripts/diagnose-cache.mjs --object pricebookitem --tenant harmon` should say 219 / 162 active; then invoke `sundial-cache-sync` with `{ "mode": "full", "object": "pricebookitem" }` (the script prints the command). Why: the DataLoader price-book rows are owned by Tim and invisible to the integration user under Private OWD (`salesforce/pricebook-import/README.md` → Sharing).
- [ ] **Service Club follow-ups:** per-kind member discounts ($50 off inspections / 5% off equipment as their own rules, not the office's hand); plan switches from Stripe's portal re-pointing `Service_Plan__c` (price id → plan); the annual tune-up / cleaning as a scheduled job (today: the "owed a visit" list on the page); the ~24 existing members — Ben sends each a join link from Add member (source Migrated).
...
- [x] **Recording self-heal + verified keys (2026-09-10).** `POST /welcome-call/orphan-match` no longer 404s when neither the holding object nor a destination exists: it re-reads the call from Retell (fresh `recording_url`) and uploads straight to the destination key, rebuilt from the **call's own `start_timestamp`** so a repair lands on the name the log already claims. Every `Recording:` key is HEAD-confirmed before it is written — otherwise the line reads `unavailable — see ledger/CloudWatch`, replacing the old fallback of printing the expiring Retell URL. Idempotency is file-aware: a repeat run skips the status/log writes but still heals a missing recording, appending a one-line correction (never an edit) if the key changed. Download gets a bounded retry (3 attempts / 3 s / 30 s budget). 12 new tests, **816 green**, deployed, and `a1P7y00000B4iaPEAR` / `call_a15c774e989eb4e8873b58de7d1` repaired and verified.
- [ ] **TIM (decision): stop the portal Files tab from silently deleting a Welcome Call recording.** Root cause of the 2026-09-10 incident was not the Lambda — a portal bulk-delete cleared that customer's Files 66 s after the sweep filed the recording (`sundial-delete-file` ×9, 18:13:34–18:13:56 UTC), then re-uploaded a document set. The heal makes this **recoverable**; it does not make it **hard**. Options, cheapest first: (a) `sundial-delete-file` refuses — or warns on — a `Welcome Call Recording` category; (b) the Files tab marks system/compliance files undeletable in the UI; (c) rely on S3 versioning alone (bytes survive under a delete marker, but nothing tells anyone the file went missing). Worth deciding before Harmon's team is doing this at volume.
