...
- `op` — how `field` is compared (2026-09-23, D-075). Omitted = equality. `includes` = multi-select picklist membership: `?field=Customer_Type__c&value=Service&op=includes` matches `"Solar;Service"` (the Service module's customer list). On the cache path it is a PostgREST `or` of the value alone / first / last / in the middle (`;`-delimited); on the live path `INCLUDES ('value')`. The value may hold letters, digits, spaces, `,()&/.'-` (a picklist value) — anything else is `400 INVALID_FILTER_VALUE`. The shadow count skips an includes query (uncomparable).
...
| `sundial-notify` | GET /notify/config, POST+DELETE /notify/subscriptions, POST /notify/test (**also** EventBridge `sundial-notify-sweep` every 5 min — D-074) |
...
| *(secret, not a variable)* `sundial/push` | every Lambda that notifies (`lib/notify.js`) | Yes to push | Secrets Manager JSON `{ "publicKey", "privateKey", "subject": "mailto:…" }` — the VAPID pair for Web Push (D-074). Absent → the bell and Realtime still work, push is simply off. Generate once with `web-push`'s `generateVAPIDKeys()`; rotating it orphans every existing subscription (people turn push on again). |
| `REMINDER_HOUR` | `sundial-notify` | No | Local hour (0–23, in `SERVICE_TIMEZONE`) of the techs' day-before digest. Default `17`. |
| *(secret, not a variable)* `sundial/google-maps` | `sundial-service-estimate`, `sundial-service-board` | No (Street View card says "not set up" until it exists; the tech app's geofence stays "unverified") | Secrets Manager JSON `{ "apiKey": "…" }` — a Google Cloud API key with the **Street View Static API**, (since amendment 7) the **Geocoding API** and (since 2026-09-22) **Places API (New)** enabled, restricted to those three. Optional `"bias": { "lat", "lng", "radiusMeters" }` nudges address suggestions toward the office's territory. Read by `GET /service/jobs/{id}/street-view` and by the tech app's clock-in (one geocode per job, written to `Geocode_Lat/Lon/Status__c`); the execution role needs `secretsmanager:GetSecretValue` on it (the role's existing `sundial/*` grant covers it if that is how the other secrets are granted). |
...
| `SERVICE_BRAND_NAME` | `sundial-service-public`, `sundial-service-board`, `sundial-service-estimate` | No | Fallback company name when Secrets Manager `sundial/brand` has none for the tenant. Superseded 2026-09-24 by the secret below for everything on the documents. |
| *(secret, not a variable)* `sundial/brand` | `sundial-service-estimate`, `sundial-service-public` | No (documents print the fallback name, no logo, no footer links) | **The tenant's brand on every customer document** (`lib/brand.js`, 2026-09-24): JSON keyed by tenant slug, plus an optional `default` block — `{ "default": {…}, "harmon": { "companyName", "logoUrl" (https, PNG or JPG ≤ 2 MB; the hosted pages show it by URL, the PDFs embed it — fetched once per Lambda and kept a day), "tagline", "addressLine", "phone", "email", "licenseLine", "termsUrl", "termsBlurb", "clubUrl", "clubBlurb", "footerNote", "accentColor" } }`. The footer prints **Terms & Conditions** (`termsUrl` + `termsBlurb`, a generic line when blank) and **Solar Service Club** (`clubUrl` + `clubBlurb`) on the estimate, invoice and job report — page and PDF alike. Re-read every five minutes; a missing secret is not an error. |
...
### `PATCH /service/estimates/{id}` — the `estimate:{}` keys above at top level. `address` (→ `Address_at_Creation__c`, 2026-09-18) corrects the service address on the estimate; the job's is corrected through the generic `PATCH /sf/job` (the portal also resets `Geocode_Status__c` → Pending and clears `Street_View_Image_Key__c` so the map and the house re-fetch); a Flow mirrors job ↔ estimate. 409 `ESTIMATE_INVOICED` once billed. Unknown keys are reported in `rejectedFields`, not written.
...
### `POST /service/customers` (the Service module's New Customer / Add to Service, D-075) → 201 | 200
Body: `customer` (above — `{ id }` or `{ new: {…}, confirmNew? }`, the same duplicate guard and Service tagging as the popups) and optional `request: { requestType, description, assignedTo (Sundial_User__c id), nextFollowUp (YYYY-MM-DD), leadSource }`. No estimate, no job. After the select-or-create the Service pipeline is opened: `Service_Stage__c = New` when the customer is new or has no stage yet (an existing stage is never overwritten), `Service_Request_Type__c` / `Lead_Source__c` matched against the org's picklists, `Description__c`, `Assigned_To__c` + `Assigned_Date__c` (today), `Next_Follow_Up_Date__c`. Every Service field is describe-guarded: an org without `salesforce/service-customer-2026-09-23/` still gets its customer and a warning naming the package. 201 when created, 200 when an existing customer was tagged; 409 `DUPLICATE_CANDIDATES` as on the popups. Response `{ success, customerId, customerCreated, serviceStage, warnings }`. Activity: `customer_created` / `customer_tagged` (`details.service = true`). Action `service.estimate.write`. Wired by `scripts/wire-service-customers-route.ps1`.
...
### Address lookup (customer create in the Service module, 2026-09-22)
`GET /service/address/suggest?q=…&session=…` → `{ status: "ok", suggestions: [{ placeId, main, secondary, text }] }` (up to 6; `q` under 3 characters answers an empty list without asking Google) | `{ status: "unconfigured", suggestions: [] }` (no key — the form just types). `GET /service/address/place/{placeId}?session=…` → `{ status: "ok", address: { street, city, state, postalCode, formatted, lat, lng } }` — street = number + abbreviated route (+ `#unit`), state the two-letter code, ZIP without the +4. Google Places API (New), proxied by `sundial-service-estimate/address.js` with the key from **`sundial/google-maps`** — never in the browser. `session` is a token the browser mints per address entry (letters / digits / `-` / `_`, 8–64) so Google bills one session per address, not one request per keystroke. `400 INVALID_PLACE`, `502 ADDRESS_LOOKUP_FAILED` (Google did not answer; the browser keeps what was typed). Action `service.estimate.write`. Wired by `scripts/wire-address-lookup-routes.ps1`. Portal: `AddressLookup` under the Street box of the New Estimate / New Job popup's "New customer" form.
...
- `GET /service/board?from=<iso>&to=<iso>[&techId=]` → `{ window, techs:[{id,name,level}], techsSource, calls:[BoardCall], unscheduled:[TrayJob], unscheduledCalls:[BoardCall], defaults:{callMinutes,timeZone} }`. **`unscheduledCalls` (2026-09-15)** = calls with `Status__c = Unscheduled` — created "schedule later", each its own tray card (with its tech if one was picked); a job that has one is left out of `unscheduled` so it is not in the tray twice. Dropping such a card schedules THAT call (PATCH with `start`), keeping its notes and tech. Window capped at 42 days — the portal's month view is a fixed 6×7 grid (`400 WINDOW_TOO_WIDE`). **Techs** = active `Sundial_User__c` with `Access_Level__c = Technician` or `Default_Department__c = Service`; until the tenant marks anyone, every active user is a column and `techsSource` is `"all-users"` (the portal shows a banner). **Tray** = jobs whose status is one of the five unscheduled statuses, emergencies first then by age, 200 max. `BoardCall` = `{ id, number, jobId, jobNumber, jobStatus, customerId, customerName, address, phone, issueDescription, paymentStatus, invoiceStatus, invoiceState, priority, serviceType, billToType, techId, techName, visitType, subType, start, end, status, cancelReason, actualStart, actualEnd, durationMinutes, workNotes, privateNotes, billable, billableHours, billRate, createdAt, modstamp }`. **The card's border + hover card (2026-09-23):** `issueDescription` / `paymentStatus` are the job's; `invoiceStatus` is the job's live (non-void) `Sundial_Service_Invoice__c.Status__c`, looked up once per board read for the jobs in the window (a broadcast of one call carries `null`); `invoiceState` = `invoiceStateFor()` — `paid` (job Paid / Payment_Status Paid / invoice Paid) · `sent` (invoice Sent / Partially Paid) · `invoiced` (invoice Issued / Draft, or job Status Invoiced) · `not_invoiced`. The portal paints the border red / amber / amber / green.
- `GET /service/jobs/{id}/calls` → `{ jobId, jobStatus, geocode: { lat, lng, status } | null, calls, techs, defaults }` (the job page's card). Every call (here and on the board) carries `clockLog: { intervals: [{ kind: "en_route" | "on_site", in, arrived, out, gps: { in, arrived, out } }], geofenceVerified }` (2026-09-19) — the live intervals with the phone's GPS at each tap, so the office sees where the tech clocked; removed intervals stay in `GET /service/calls/{id}/clock`.
...
**A Complete call's notes roll up onto the job (2026-09-19, `sundial-service-board/job-notes.js`).** When a call becomes Complete — the tech's tap, the board's status menu, or a time correction with `complete` — its `Work_Notes__c` are copied onto the job's `Notes_for_Summary__c` and its `Private_Notes__c` onto `Notes_From_Service_Calls__c`, one block per call headed `— SC-00012 · Sep 18, 2026 · Larry Ng`. Editing a Complete call's notes (the board's PATCH, the tech's later note) REPLACES that call's block; emptied notes remove it; text the office typed ahead of the blocks is kept. Best-effort: never fails the tap; the job's cache is marked stale and a `job_updated` activity row carries `callNotesRolledUp`. Nothing reaches the job before completion.

**The tech app's reads gained (2026-09-19):** `remoteDiagnosis` (`Initial_Remote_Diagnosis__c`) and `streetViewUrl` (the job's cached Street View still) on `GET /service/tech/jobs/{id}` → `job` and on `GET /service/tech/calls/{id}` → `call`; `GET /service/tech/jobs/{id}/street-view` (`service.tech.read`, served by the ESTIMATE Lambda — same handler as the office's, refresh disabled) → `{ status: "ready", url } | { status: "none" | "no_address" | "unconfigured" }`; `GET /service/tech/customers?type=Solar|Roofing|Commercial|Service` filters on `Customer_Type__c` (multi-select INCLUDES; an unknown value is ignored, the answer echoes `type`), and each customer carries `customerType`.

**The customer's job report + receipt (2026-09-19, D-072 amendment 10, `report.js`).** `GET /service/jobs/{id}/report` (`service.estimate.write`) → `{ jobId, jobNumber, sections, receipt, intro, sectionCount, updatedAt, sentAt, sentCount, editedSinceSent, publicUrl, pdfUrl, partnerBilled, summary, photos: [{ key, fileName, publicUrl, size, lastModified, callId, callNumber, visitLabel }], invoice: { number, status, total, paid } | null }` — the saved sections plus every photo on the job to pick from. `PUT …/report` `{ sections: [{ id?, photoKey?, caption }], receipt?, intro? }` (`service.estimate.write`) → the state above; `400 REPORT_INVALID` (shape / caps), `400 PHOTO_NOT_ON_JOB` (a key outside `SUNDIAL/{jobId}/photos/`). `GET …/report/preview` → `{ html, title, sections, receipt }` (PREVIEW watermark). `POST …/report/send` `{ via?: "Email" | "SMS" | "Both", to?, toPhone?, message? }` (`service.estimate.send`) → `{ delivery: "email" | "sms" | "both" | "recorded", recipient, textedTo, deliveryDetail, …state }` — renders `SUNDIAL/{jobId}/job-report-{n}.pdf` (photos embedded), emails the hosted link + the PDF (attached under 8 MB, linked above), texts the link through the SMS sender; `400 REPORT_EMPTY` with no sections and no Summary of work. **Public:** `GET /public/reports/{token}` (sundial-service-public, no auth) → `{ html, title, jobNumber, customerName, sentAt, pdfUrl, sections, receipt, paid }`; 404 unknown / malformed, 410 expired. The portal page is `/report/{token}`.

**The job's photos + files (2026-09-18).** Photos live in ONE folder per job — `SUNDIAL/{jobId}/photos/{callId}/…` for the visits, `SUNDIAL/{jobId}/photos/…` for what the office adds — so every tech on the job sees every visit's photos and the office sees them all on the job page. `GET /service/jobs/{id}/photos` (`service.board.read`) and `GET /service/tech/jobs/{id}/photos` (`service.tech.read`) → `{ jobId, jobNumber, total, groups: [{ callId | null, callNumber, techName, start, label, photos: [{ key, fileName, publicUrl, size, lastModified, callId }] }] }` — the office's folder first, then visits newest first. `POST /service/jobs/{id}/photos` `{ fileName, contentType, size? }` (`files.job.upload`, images only, 25 MB) → `{ uploadUrl, key, publicUrl, expiresIn }` presigned straight under `photos/`; `POST …/photos/confirm` `{ key, contentType?, size?, caption? }` registers the `sundial_file_metadata` row (`category: photo`, `subfolder: photos`) and refuses a key inside a call's folder (`400 KEY_INVALID`). `GET /service/tech/jobs/{id}/files` (`service.tech.read`) → `{ files: [{ key, fileName, publicUrl, size, lastModified, source: "job" | "estimate" }] }` — the job's folder minus the photos, plus the estimate's folder (the sent PDFs); read-only.
...
## Notifications (D-074, 2026-09-21) — `sundial-notify` + `lib/notify.js` in every emitter

The bell, browser pop-ups and Web Push (`docs/integrations/push-notifications.md`). **The rows are Supabase `sundial_notifications`**, one per person per event, written only by the Lambdas through `lib/notify.js` and read by the browser directly under RLS (`profile_id = auth.uid()`; the browser may update `read_at` and nothing else). Every insert is also broadcast on **`user:{profile_id}:notify`**, event `notification`, payload `{ id, title, body, url, category, kind, createdAt }`, and pushed to the person's devices. Recipients are auth uuids; a Salesforce user id is translated through `profiles.sundial_user_id`. Categories: techs `schedule | reminder | customer_text | mention`; office `tech_activity | money | customer_message | mention`. Per-person switches in `user_preferences.notify_prefs` (`{ "<category>": false, "browser": false }`; a missing key is ON). Dedupe: unique `(profile_id, dedupe_key)`.
- `GET /notify/config` → `{ pushConfigured, publicKey }` — the VAPID public key (never the private one). Action **`notify.self`** (every signed-in scope but `none`; the routes only touch the caller's own rows).
- `POST /notify/subscriptions` `{ subscription: { endpoint, keys: { p256dh, auth } } | endpoint, keys, app?: office|tech }` → `{ success, app, devices }` — turn push on for THIS browser / phone. The row is stamped with the verified JWT subject, never request input; the same endpoint re-subscribed replaces the row (and follows whoever is signed in on that device). `400 SUBSCRIPTION_INVALID | ENDPOINT_INVALID`, `503 PUSH_NOT_CONFIGURED`.
- `DELETE /notify/subscriptions` `{ endpoint }` → `{ success, removed }` — only the caller's own row.
- `POST /notify/test` → `{ success, inserted, pushed }` — a bell row + a push to each of the caller's devices.
- **The sweep** (EventBridge rule `sundial-notify-sweep`, `rate(5 minutes)`, the default scheduled-event payload; `{ "sweep": true }` runs it by hand): `reminder/one_hour` — a tech's `Scheduled` call starting within 65 minutes; `reminder/day_before` — once per tech per local day at `REMINDER_HOUR`, tomorrow's calls in one line; `tech_activity/late` — a `Scheduled` call 30+ minutes past its start (within 12 h) with no tap, to the office. Keys `reminder:one_hour:{call}`, `reminder:day_before:{tech}:{day}`, `late:{call}` — a sweep that re-finds the same calls rings nobody twice. Returns `{ ok, oneHour, dayBefore, late, techsReminded }`.
- **Emitters elsewhere** (each `dedupeKey` in parentheses): board `POST /service/jobs/{id}/calls`, `PATCH /service/calls/{id}`, `POST …/cancel` → the tech, `schedule/scheduled|moved|reassigned_away|cancelled` (`schedule:{kind}:{call}:{tech}:{start}`); tech `POST /service/tech/calls/{id}/status` `In Progress | Complete | No-Show` → the office, `tech_activity/clock_in|complete|no_show` (`tech:{status}:{call}:{eventId}`) — "on my way" is not an office alert; `POST /sms/inbound` → the office `customer_message/text` and the tech on that job today `customer_text/text` (`sms:{MessageSid}`); public `accept` / `decline` → the office `money/estimate_approved|estimate_declined` (`money:approved:{est}:{version}`); Stripe `payment_intent.succeeded|payment_failed` → the office `money/deposit_paid|invoice_paid|payment_failed` (`money:paid|failed:{pi}`); the club's team emails → the office `money/club_join|club_ended|club_past_due|club_cancel_scheduled`, `customer_message/booking|call_me`; `POST /webhooks/comment-mention` → the mentioned person `mention/comment` (`mention:{id}`), regardless of the email switch.
- Wire script: `scripts/wire-notify-routes.ps1` (routes + the EventBridge rule). SQL: `sql/sundial_notifications.sql`.

## Service Club (D-073, 2026-09-18) — `sundial-service-estimate/club.js`

Runbook: `docs/integrations/service-club.md`. The catalog is `Sundial_Service_Plan__c` rows; memberships are `Sundial_Membership__c` rows that follow a Stripe subscription. Actions `service.club.read` / `service.club.write` (tenant scope).

**Public (no login; `{tenant}` is the slug, `Sundial_Tenant__c.Name`; unknown slug → 404 `TENANT_UNKNOWN`):**

- `GET /public/club/{tenant}/plans` → `{ tenant, brand, configured, plans: [{ code, name, kind: "subscription", availability, purchasable, highlight, tagline, features[], monthly, yearly, discountDescription, includes: { tuneUp, cleaning } }], oneTime: [{ …, kind: "one-time", price }] }` — Available + Coming Soon rows only (Coming Soon is `purchasable: false`); `configured` false when the tenant has no Stripe keys.
- `POST /public/club/{tenant}/join` `{ planCode, interval: "monthly"|"yearly", customer: { firstName, lastName, email, phone, street, city, state, postalCode }, notes? }` → **200** `{ url, membershipId, plan, interval, price, mode }` — the customer is matched by email / phone / address or created (tagged Service), a **Pending** membership is written, and `url` is a Stripe Checkout Session (`subscription` mode) the browser goes to. **400** `JOIN_INVALID { problems[] }`; **404** unknown plan; **409** `PLAN_NOT_AVAILABLE`, `PLAN_INTERVAL_UNAVAILABLE`, `ALREADY_MEMBER { membershipId, plan }`; **503** `STRIPE_NOT_CONFIGURED` / `PUBLIC_URL_NOT_SET`.
- `GET /public/club/{tenant}/joined?session=cs_…` → `{ status, number, plan: { code, name, includesTuneUp }, interval, price, firstName }` — what the success page may show; the page polls it until `Active`.
- `POST /public/club/{tenant}/truck-roll` `{ customer, issue?, planCode? = "truck-roll" }` → **200** `{ url, jobId, estimateId, amount, mode }` — an estimate **Approved** (Online, deposit = the plan's price) + a job (`Intake_Channel__c Web Form`, `Needs_Intake_Review__c`, `Service_Type__c Paid Service`) with one Approved line (the plan's price-book item by code, else an ad-hoc Fee at `Price__c`), and a `payment`-mode Checkout carrying the existing `kind: "deposit"` metadata — the payment lands through the payments branch of the webhook untouched. The team is emailed.
- `POST /public/club/{tenant}/request` `{ customer, message? }` → **201** `{ success, jobNumber }` — "call me": a job in `Needs_Intake_Review__c` with the message as `Issue_Description__c`; the team is emailed.
- `POST /public/club/{tenant}/manage` `{ email }` → **200** always `{ sent: true, message }` — when the email belongs to a customer with `Active_Membership__c`, a Stripe **customer-portal** session is created and its URL is emailed to that address; the response never says whether the email was known.

**Webhook (the existing `POST /webhooks/stripe/{tenant}`):** `checkout.session.completed` with `mode: "subscription"` → the membership (by `metadata.membershipId`, else the session id) goes Active (`Stripe_Subscription_Id__c`, `Started_At__c`, `Current_Period_End__c`), the customer's `Active_Membership__c` is set, SolarFax is sent the member + connect invite (their API, `lib/solarfacts.js`; the Zapier hook if only that is configured; ids stamped in `SolarFacts_Account_Id__c` / `SolarFacts_User_Id__c`), the team emailed; `customer.subscription.updated` → status / period end / cancel-at-period-end mirrored; `customer.subscription.deleted` → Cancelled + `Ended_At__c`, pointer cleared, SolarFax `disconnect`; `invoice.paid` → `Last_Payment_*`, `Lifetime_Revenue__c` (+ Past Due → Active); `invoice.payment_failed` → Past Due, `Payment_Failures__c` + 1, team email. Ledger rows carry `membership_sf_id`. An unknown subscription is `ignored`, never created.

**Office:**

- `GET /service/club/plans` → `{ plans: [planView], stripeConfigured, stripeMode }`; `PATCH /service/club/plans/{id}` `{ name?, availability?, sortOrder?, highlight?, tagline?, features?: string[], monthlyPrice?, yearlyPrice?, price?, priceBookItemCode?, discountScope?, discountType?, discountValue?, discountDescription?, includesTuneUp?, includesCleaning?, notes? }` → `{ success, plan, rejectedFields, warnings }` — a changed subscription price mints a new Stripe Price and archives the old (`warnings` says when Stripe could not be updated).
- `GET /service/club/memberships?status=&q=` → `{ memberships: [{ id, number, status, customerId, customerName, email, phone, address, plan: { id, code, name, discountDescription }, interval, price, source, startedAt, currentPeriodEnd, cancelAtPeriodEnd, cancelledAt, endedAt, cancelReason, lastPaymentAt, lastPaymentAmount, lifetimeRevenue, paymentFailures, solarFacts: { status, lastSentAt, lastError }, stripeSubscriptionId, notes, createdAt }], plans }` (newest first, 500 max; `status=all` or one status).
- `POST /service/club/memberships` `{ customerId, planCode | planId, interval, source?: "Office"|"Migrated", email?: boolean, notes? }` → **201** `{ membershipId, url, emailed, plan, interval, price, mode }` — a Pending row + the join link; `email: true` sends it to the customer's email. Same 409s as the public join.
- `POST /service/club/memberships/{id}/cancel` `{ reason?, immediately? }` → `{ success, status, cancelAtPeriodEnd, currentPeriodEnd, solarFacts }` — through Stripe (`cancel_at_period_end`, or DELETE); a Pending row becomes Expired without touching Stripe; **409** `MEMBERSHIP_ENDED`.
- `POST /service/club/memberships/{id}/solarfacts` → **200** `{ success: true, event, via: "api" | "hook", membership }` / **502** `{ success: false, reason }` — redoes the hand-off (`member.activated` → invite, or `member.cancelled` → disconnect for an ended row).
- `GET /service/club/report` → `{ total, active, byStatus, byPlan: [{ planId, plan, total, <status>: n }], mrr, revenue: { last30Days, thisMonth, yearToDate, byMonth: [{ month, amount }] }, owedVisits: [{ membershipId, number, customerId, customerName, plan, tuneUp, cleaning, startedAt }], plans }` — revenue from the ledger's applied `invoice.paid` rows.
- `GET /service/club/customers/{customerId}` → `{ customerId, active: membership | null, discount: { Discount_Scope__c, Discount_Type__c, Discount_Value__c } | null, history: [membership] }` — the customer / estimate / job pages' member chip.
- `POST /service/estimates/{id}/apply-plan-discount` → `{ success, totals, plan, discount }` — the member's plan discount onto an existing estimate (`Discount_Source__c Service Plan`, `Membership__c`); **409** `NOT_A_MEMBER`, `PLAN_NO_DISCOUNT`, `ESTIMATE_INVOICED`. A **new** estimate for a member gets it automatically unless the request set its own discount.
