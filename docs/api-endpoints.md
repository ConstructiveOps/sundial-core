...
| `sundial-service-board` | GET /service/board, GET+POST /service/jobs/{id}/calls, PATCH /service/calls/{id}, POST /service/calls/{id}/cancel |
| `sundial-service-public` | GET /public/estimates/{token}, POST /public/estimates/{token}/{accept\|decline} (no auth — the token is the credential) |
...
| *(secret, not a variable)* `sundial/google-maps` | `sundial-service-estimate` | No (Street View card says "not set up" until it exists) | Secrets Manager JSON `{ "apiKey": "…" }` — a Google Cloud API key with the **Street View Static API** enabled and restricted to that API. Read by `GET /service/jobs/{id}/street-view`; the execution role needs `secretsmanager:GetSecretValue` on it (the role's existing `sundial/*` grant covers it if that is how the other secrets are granted). |
...
Wire script: `scripts/wire-service-board-routes.ps1` (adds `/service/board`, `/service/jobs/{id}/calls`, `/service/calls/{id}`, `/service/calls/{id}/cancel`; invoke permission `apigw-service-board` on `…/*/*/service/*`).
...
- `POST /service/jobs/{id}/sms` `{ body (≤1600), to? }` → `{ success, message }` — sends through Twilio from the tenant's number with a status callback, stores the row as `queued`. `400 EMPTY_BODY | BODY_TOO_LONG | NO_PHONE`; `503 SMS_NOT_CONFIGURED`; `502 SMS_SEND_FAILED` (the row is kept as `failed` with Twilio's error code, so the office sees it). Action **`service.sms.send`**.
...
Wire script: `scripts/wire-service-public-routes.ps1` (resources `/public/estimates/{token}` + `/accept` + `/decline`, invoke permission `apigw-service-public` on `…/*/*/public/*`).
