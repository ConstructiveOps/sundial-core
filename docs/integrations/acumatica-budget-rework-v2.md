...
| D21 | ~~**COMMISSION BURDEN = 75% × (management + setter) ONLY.**~~ ⚠️ **PARTIALLY REVERSED 2026-09-08 BY D35 — the internal rep IS burdened again. The rest of this row still stands (the external rep is not burdened, and the fixture job is unaffected). Read D35 before acting on anything below.** **Neither rep line is burdened** — not the external one (never was) and **not the internal redline commission** either. This **amends D9 and the D19 Stage 2 implementation**, both of which burdened the internal rep amount, and it **supersedes the REVISED sheet's J12**, whose burden array includes K8 (the internal rep cell). The sheet is not to be "restored" here: under the redline model the internal rep amount is an order of magnitude larger than when that array was written, and Harmon has ruled. Effect on the fixture job: internal-deal burden 10,939.50 → **415.50**, identical to the same job sold externally. Nothing else moves — the external worked example was already 75% × (mgmt + setter). | Harmon / Tim, 2026-08-22 |
...
| D34 | **STAGE E NOW WRITES JOBTYPE AND REFRESHES THE PROJECT MANAGER ON EVERY BUDGET PUSH — and both definitions moved to `lib/`.** Supersedes the exclusion recorded in D31's own comment and in `runDownstreamStages`: *"RS vs RSDC is authoritative at Layer-1 creation and this worker only infers it — inference is not authority."* That argument assumed JOBTYPE tracks RS vs RSDC. **It does not** (D31): the allowed ValueIDs are `CE CS EV RE RS SE`, there is no RSDC job type, and every live RSDC-template project carries `RS`. It is a constant, so nothing is inferred and there is no authority to defer to. **The PROJECT MANAGER is refreshed here for a reason Layer-1 structurally cannot cover: managers are assigned AFTER the project exists**, and Layer-1 only ever sees the field at create time, when it is usually still blank. Both ride the SAME `Project` PUT that already carries the attributes, and both are proven by the SAME verifying re-read — one round trip, one proof, nothing that can half-succeed. ⚠️ **THE REFRESH NEVER CLEARS.** Blank field, unmapped name and two-mapped-names all resolve to "say nothing", so `ProjectProperties` is omitted from the body and the merge (D24) leaves whatever is there alone. Clearing on a blank would be worse than not running: there is one manager slot, most Solar records carry a retired name that maps to no Acumatica employee, and the first push after go-live would strip the manager off every one of them. An unmapped name is a **note on a pushed budget** (`Budget_Push_Error__c`, status still `Pushed`), never a failed push — same discipline as every other downstream problem, and it does NOT mark the attribute sync as `Failed`, because those are separate facts. **Shared definitions, not copies:** `JOBTYPE_ATTRIBUTE_ID`/`JOBTYPE_VALUE` in `lib/acumatica-attributes.js` and `normalizePicklist`/`PROJECT_MANAGER_EMPLOYEE_IDS`/`resolveProjectManager` in the new `lib/acumatica-project-manager.js`, imported by both Lambdas — two copies of a name map eventually become two different name maps, and the failure is silent (a job created under one spelling and refreshed under the other simply stops having a manager). **D26 is NOT widened:** the attribute-only path still sends neither, because its whole claim to safety on legacy projects is that it writes only what it was asked to; back-filling legacy JOBTYPE is a decision to take deliberately, not a side effect of this one. | Tim, 2026-09-08 |

| D35 | **COMMISSION BURDEN = RATE × (MANAGEMENT + SETTER + INTERNAL REP) — PARTIALLY REVERSES D21, restoring D19 Stage 2's basis.** Harmon ruling 2026-09-08 (ADR **D-071**). **The principle, now stated rather than implied: burden follows PAYROLL, and only payroll.** An internal rep is paid through payroll — D16 is explicit that internal deals raise no PO for exactly that reason — so the employer costs burden represents are real. An external rep is paid by a **dealer purchase order**: no payroll, no employer tax, nothing to burden. Management and the setter are payroll and always were. **D21's error was treating the two rep lines as one category because both are "rep commission"; they are two different payment MECHANISMS, and burden is a fact about the mechanism, not about who earned it.** Third ruling on this line — D19 Stage 2 (2026-08-21) included the internal rep, D21 (2026-08-22) removed both reps, D35 restores the internal one — so `budgetCalc.js` now carries the full sequence in a comment rather than just the current answer. **THE SHEET AGREES AGAIN:** the REVISED workbook's J12 always included K8, and D21's standing warning not to "restore" it is withdrawn; what remains true is the magnitude note, because under the redline model the internal rep term now dominates an internal job's burden. **EXTERNAL JOBS ARE UNCHANGED BY CONSTRUCTION** — the term is `internalComm`, which the existing deal-type routing already sets to zero on an external deal (reused, not re-derived), so every external job computes exactly as under D21 and the HOLLAND fixture's 415.50 does not move. That fixture is therefore the regression proof that external did not move **and is blind to which of the three rules is in force**; the internal-deal behaviour tests are the only thing pinning the basis. **Worked delta on the fixture job sold internally:** burden **415.50 → 10,939.50** (+10,524.00 = 0.75 × 14,032), total commissions 15,001.50 → 25,525.50, job cost with commission 39,558.98 → 50,082.98, balance of revenue 21,500.50 → 10,976.50, GP$ −3,056.98 → −13,580.98. The `BURDENEXR · SALESCOMM` budget line reads `Commission_Burden_Amt__c` directly, so it needs no arithmetic change — but its mapping note did, and has it. `BURDENEXR · RESIDENTAL` (labor burden) is untouched and a test pins that the two do not leak into each other. | Harmon / Tim, 2026-09-08 |
...
| JOBTYPE | Job Type — a **Combo**; allowed ValueIDs `CE CS EV RE RS SE`. **There is no RSDC value** (D31) | the constant `JOBTYPE_VALUE` = **`RS`** (`lib/acumatica-attributes.js`) | Layer-1 create **and every budget push** (D34) |
| KW | Kilowatts | System_Size__c | Layer-1 / budget push |
| *(not an attribute)* | Project Manager → `ProjectProperties.ProjectManager` | `Project_Manager__c` via the shared name map | Layer-1 create **and every budget push** (D34) |
