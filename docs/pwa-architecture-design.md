...
> **Status (2026-09-16): the original design brief, kept for the record.** The app was built
> (D-072 amendment 7) and `docs/pwa-architecture.md` describes it *as built*. Where the two
> differ, the as-built page wins: the app lives at `/tech` (not `/field`); writes replay from
> a queue in the page against the ordinary `/service/tech/*` routes with an `eventId` per tap
> (no `sundial-field-sync` Lambda or idempotency table); geocoding is Google on the existing
> key (not AWS Location); Web Push is still open.
