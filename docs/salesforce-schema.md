...
- `Energy_Rate__c` — Currency ← `financing.solar_rate`, the customer's **$/kWh energy rate**. **Not** `Solar_Price_per_Watt__c` — that is contract amount ÷ system watts, a different metric this pipeline writes as `Contract_Price_Per_Watt__c`. ⚠️ The field is Currency with **scale 2**, so a rate like `$0.1425/kWh` stores as `0.14` — a ~1.8% error on a number that feeds customer-facing math. Widening it to 4 decimal places is a one-field change in Setup and worth doing before lease/PPA volume arrives.
