...
Lease/PPA-only financing fields (verified present by describe 2026-08-17; written on the non-loan branch, absent for cash and loans):
- `Energy_Rate__c` — Currency ← `financing.solar_rate`, the customer's **$/kWh energy rate**. **Not** `Solar_Price_per_Watt__c` — that is contract amount ÷ system watts, a different metric this pipeline writes as `Contract_Price_Per_Watt__c`. ⚠️ The field is Currency with **scale 2** (re-verified against the live org 2026-08-17 with a forced-refresh describe), so a rate like `$0.1425/kWh` stores as `0.14` — a ~1.8% error on a number that feeds customer-facing math. Widening it to 4 decimal places was attempted but **has not taken effect in the org**; see TASKS.md. Re-check with `describeObject('Sundial_Customer__c', { forceRefresh: true })` after saving.
- `Escalator__c` — Percent(4,2) ← `financing.escalation`. ⚠️ **Unit unverified.** A Salesforce percent field stores the percentage itself (`2.9` = 2.9%); Aurora's docs don't state whether `escalation` is a percentage or a fraction, and Aurora is inconsistent elsewhere (`annual_offset` is the string `"87%"`). The value is written **unconverted** — never a guessed ×100 — and the worker warns when it is `0 < x < 1`, the tell for a fraction. Note the precision ceiling too: `(4,2)` caps the field at `99.99`, which is ample for a percentage and would silently be wrong for a fraction.
- `upfront_payment` — **deliberately unmapped.** Aurora defines it nowhere, and the plausible readings (prepayment reducing the monthly vs. due-at-signing fee) belong in different fields. Not written until a real Participate payload proves its meaning (TASKS.md).
...
**Pipeline position on `signed` — applies to EVERY signed agreement (Tim, 2026-08-10).** Aurora's `signed` status translates to `Status__c` = **`Customer`** and `Stage__c` = **`Sold - Pending Review`** in Sundial, written by **both** paths: a customer auto-created from a dealer project (at insert) and a pre-existing customer matched by `Aurora_Project_ID__c` (on the signed write-back). Both values exist in the org (verified by describe). One shared helper (`customerCreate.js » buildSignedPipelineFields`) produces them so the two paths cannot drift.

`Status__c` is load-bearing — the org default is **`Lead`**, so without it a closed sale would sit in the CRM as a lead. **`Stage__c` is the notification mechanism:** Harmon's Salesforce alerts trigger off that Stage, which is why the SES email channel is deliberately left unconfigured. If the picklist value is ever renamed, the write is skipped with a warning **and those alerts silently stop firing** — the warning says so explicitly.

Not written on any other status: a non-signed event never moves the pipeline, a confirmed cancellation does not promote a dead contract, and a `signed` event that Aurora contradicts on re-read records Aurora's status only.
