...
export const MAPPING_ROWS = [
  ...
  { line: "Commission Burden", taskId: "BURDENEXR", accountGroup: "LABOR", type: "Expense", inventoryId: "SALESCOMM", amountField: "Commission_Burden_Amt__c", hoursField: null, keyStatus: "harvested", note: "D-071 (2026-09-08, PARTIALLY REVERSES D21): rate x (management + setter + INTERNAL rep). Burden follows PAYROLL and only payroll - an internal rep is paid through payroll (D16: internal deals raise no PO for exactly that reason), an external rep is paid by a dealer purchase order, so only the internal one carries employer cost. Ruled three times now: D19 Stage 2 included the internal rep, D21 removed it, D-071 restores it. An EXTERNAL job is unchanged under all three, because its internal-rep term is zero. The amount is read from the single Commission_Burden_Amt__c field, so this row needs no component arithmetic and the ruling reaches it through the calc - but the note is what a reader checks the number against, so it has to be right. InventoryID SALESCOMM separates it from the RESIDENTAL labor burden." },
