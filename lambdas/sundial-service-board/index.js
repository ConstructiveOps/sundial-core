...
export const CALL_STATUSES = Object.freeze(["Scheduled", "En Route", "In Progress", "Complete", "Cancelled", "No-Show"]);
...
export const CALL_SELECT =
  ...
  "Private_Notes__c, Geofence_Verified__c, Photos_Count__c, SystemModstamp, CreatedDate, " +
...
export function createHandler(deps = {}) {
  ...
  const H = {
    ...
    async board({ ctx, query }) {
      ...
      const [{ techs, source }, calls, unscheduled] = await Promise.all([
        loadTechs(tenantId),
        d.sfQuery(
      ...
      return jsonResponse(200, cors, {
        ...
        unscheduled: sortTray((unscheduled || []).map((j) => jobToTray(j, now))),
    ...
    async createCall({ ctx, params, body }) {
      ...
      const tech = await loadTech(techId, tenantId);
      ...
      await act(ctx, {
        ...
        jobSfId: job.Id,
        estimateSfId: job.Estimate__c ?? null,
        details: { techId: tech.Id, techName: techName(tech), start, end, subType },
      });
      const calls = await loadJobCalls(job.Id, tenantId);
      const jobStatus = await settleJobStatus(ctx, job, "scheduled", calls);
      await markStale(CACHE.call, [created.id], tenantId);
      let notify = { notified: false, detail: null };
      if (body?.notifyCustomer === true) notify = await notifyCustomer(ctx, { kind: "scheduled", job, call: call || fields, tech });
      const shaped = call ? callToBoard(call) : { id: created.id, jobId: job.Id, techId: tech.Id, start, end, status: "Scheduled" };
    ...
    async patchCall({ ctx, params, body }) {
      ...
      if (has("start") || has("end")) {
        ...
        const end = has("end") ? isoOrNull(body.end) : call.Scheduled_End__c;
      ...
      if (call.Sundial_Service_Job__c && (changes.Status__c || moving)) {
        ...
        if (job && changes.Status__c) {
          const calls = await loadJobCalls(job.Id, tenantId);
          if (changes.Status__c.to === "In Progress") jobStatusChanged = await settleJobStatus(ctx, job, "in_progress", calls);
      ...
      if (body?.notifyCustomer === true && moving) {
        ...
        notify = await notifyCustomer(ctx, { kind: "updated", job, call: after, tech: tech || after.Tech__r });
      }
