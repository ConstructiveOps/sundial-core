...
export function assertAction(actionKey, access) {
  ...
  // A route may name SEVERAL keys, any one of which admits the caller (a read the
  // office reaches through service.estimate.write and a tech through service.tech.read).
  const keys = Array.isArray(actionKey) ? actionKey : [actionKey];
  if (keys.some((k) => canAction(k, access))) return null;
