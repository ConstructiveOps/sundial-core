...
//
// WIRED AND SERVING as of Phase 3 (2026-08-28): sundial-sf-query applies rowFilter /
// userFilter to every read when ACCESS_MODEL_MODE=enforce. Phase 1 built and tested this
// module; Phase 2 ran it in shadow against live traffic for the evidence; Phase 3 serves
// it. What a sales role sees is now decided HERE, and nowhere else.
//
// ---------------------------------------------------------------------------
...
export const SCOPES = Object.freeze({
  ...
  DEALER: "dealer",
  OWN: "own",
  // A technician (2026-09-15, the tech app): NOT a row-visibility class over the
  // objects below — a tech reads nothing through the generic /sf routes — but a scope
  // that may perform the `service.tech.*` actions, each of which checks that the call
  // is THEIR call (Tech__c = the user) in the Lambda. Kept distinct from `none` so the
  // difference between "a technician" and "nobody set a level" survives into
  // /auth/me, the profile columns and the logs.
  TECH: "tech",
  NONE: "none",
});
...
export const SCOPE_BY_ACCESS_LEVEL = Object.freeze({
  ...
  // Phase II (Service, 2026-09-15): a Technician gets the `tech` scope — the tech app's
  // own routes, nothing through the generic object reads. Before the tech app existed
  // this was `none` (a deliberate narrowing from the Phase 0 finding that a Technician
  // saw everything); `tech` keeps that narrowing for every object and opens exactly the
  // service.tech.* actions.
  Technician: SCOPES.TECH,
});
...
export const OBJECT_ACCESS = Object.freeze({
  ...
  // Service Operations (Phase 2, D-072 - seven objects): an office/tech module. salesScopes false -
  // sales roles and dealers have no service surface (same reasoning as roofing:
  // the module gate is a separate decision from what the data model could filter;
  // revisit only when Ralph's commissioned service-rep model lands, deliberately).
  ...
  // looks like an answer. userFilter() below is the pair that answers it.
  user: Object.freeze({
...
export const ACTION_SCOPES = Object.freeze({
  ...
  // Roofing and PO files — tenant only, like the modules themselves. These two keys
  // were in lib/file-access.js's allowlist from the start but never had action rows,
  // so the Files tab on a Roofing project answered 403 to everyone (found 2026-09-11
  // while adding the service keys; the gate denies an unknown action by design).
  "files.roofing.list": Object.freeze([SCOPES.TENANT]),
  "files.roofing.related": Object.freeze([SCOPES.TENANT]),
  "files.roofing.upload": Object.freeze([SCOPES.TENANT]),
  "files.roofing.delete": Object.freeze([SCOPES.TENANT]),
  "files.po.list": Object.freeze([SCOPES.TENANT]),
  "files.po.related": Object.freeze([SCOPES.TENANT]),
  "files.po.upload": Object.freeze([SCOPES.TENANT]),
  "files.po.delete": Object.freeze([SCOPES.TENANT]),

  // Service module files (D-072): the estimate PDF per send, job documents, service-call
  // photos. Office/tech module → tenant scope; sales roles have no service surface.
  "files.estimate.list": Object.freeze([SCOPES.TENANT]),
  "files.estimate.related": Object.freeze([SCOPES.TENANT]),
  "files.estimate.upload": Object.freeze([SCOPES.TENANT]),
  "files.estimate.delete": Object.freeze([SCOPES.TENANT]),
  "files.job.list": Object.freeze([SCOPES.TENANT]),
  "files.job.related": Object.freeze([SCOPES.TENANT]),
  "files.job.upload": Object.freeze([SCOPES.TENANT]),
  "files.job.delete": Object.freeze([SCOPES.TENANT]),
  "files.servicecall.list": Object.freeze([SCOPES.TENANT]),
  "files.servicecall.related": Object.freeze([SCOPES.TENANT]),
  "files.servicecall.upload": Object.freeze([SCOPES.TENANT]),
  "files.servicecall.delete": Object.freeze([SCOPES.TENANT]),
  ...
  // Service module writes (D-072; lambdas/sundial-service-estimate). Office only —
  // tenant scope. The tech PWA gets its own deliberate actions when it lands.
  "service.estimate.write": Object.freeze([SCOPES.TENANT]),
  "service.estimate.send": Object.freeze([SCOPES.TENANT]),
  "service.job.create": Object.freeze([SCOPES.TENANT]),
  "service.pricebook.write": Object.freeze([SCOPES.TENANT]),
  // Dispatch board (lambdas/sundial-service-board): read the board / a job's calls;
  // schedule, move, reassign, progress, cancel a call. Office only for now — the tech
  // PWA gets its own narrower actions (own calls, clock events) when it lands.
  "service.board.read": Object.freeze([SCOPES.TENANT]),
  "service.call.write": Object.freeze([SCOPES.TENANT]),
  // Money (D-072.6 / .1; invoice.js inside sundial-service-estimate): issue / send /
  // void the job's invoice and record payments against it. Reads ride on
  // service.estimate.write. Its own key because "who may take money" is a question a
  // tenant will answer differently from "who may edit an estimate".
  "service.invoice.write": Object.freeze([SCOPES.TENANT]),
  // Customer texting (D-072 amendment 6; lambdas/sundial-sms): send an SMS to the job's
  // customer from the tenant's number. Reading the thread rides on
  // service.estimate.write like the other job reads. Its own key because "who may text
  // customers" is a tenant policy question (A2P rules, tone) separate from editing.
  // A tech may text the customer too (the tech app's Communications panel, amendment 7
  // follow-up): the same number, the same thread, the office sees every message.
  "service.sms.send": Object.freeze([SCOPES.TENANT, SCOPES.TECH]),
  // The tech app (D-072 amendment 7): a technician's own day — their calls, clock in /
  // out, notes, photos, checklist, field estimate lines. Every route re-checks that the
  // call belongs to the caller; the office (tenant scope) may use the same routes, e.g.
  // a manager standing in for a tech, and sees any call.
  "service.tech.self": Object.freeze([SCOPES.TECH, SCOPES.TENANT]),
  // The tech app's READ-ONLY look at the rest of the Service module (amendment 7
  // follow-up, 2026-09-16): jobs, estimates, customers and the job's Communications
  // (texts + activity) across the whole tenant — a tech at a house needs the customer's
  // history and the estimate whether or not the call is theirs. Reads only: every write
  // a tech can make is still service.tech.self (+ texting under service.sms.send), and
  // this key is what the sms / estimate Lambdas check on their read routes alongside
  // service.estimate.write.
  "service.tech.read": Object.freeze([SCOPES.TECH, SCOPES.TENANT]),
  // The Service Club (D-073; club.js inside sundial-service-estimate): see the plans,
  // memberships and the report (read); send a join link, cancel through Stripe, edit a
  // plan, resend the SolarFacts hand-off (write). Office only — a tech never touches
  // memberships. Its own keys because "who may sell or cancel a membership" is a
  // tenant policy question apart from editing estimates.
  "service.club.read": Object.freeze([SCOPES.TENANT]),
  "service.club.write": Object.freeze([SCOPES.TENANT]),
  // Notifications (D-074, 2026-09-21; sundial-notify): turn push on / off for THIS
  // device, read the VAPID public key, send yourself a test. Every signed-in scope but
  // `none` — it only ever touches the caller's own rows (the recipient is the verified
  // JWT subject, never request input), so there is nothing here for scope to protect.
  "notify.self": Object.freeze([SCOPES.TENANT, SCOPES.DEALER, SCOPES.OWN, SCOPES.TECH]),
});
...
function ctx(access) {
  ...
  if (scope !== SCOPES.TENANT && scope !== SCOPES.DEALER && scope !== SCOPES.OWN && scope !== SCOPES.TECH) {
...
export function canReadObject(objectKey, access) {
  const a = ctx(access);
  // `tech` reads no object through the generic routes — the tech app has its own
  // (service.tech.self), each scoped to the caller's calls.
  if (a.scope === SCOPES.NONE || a.scope === SCOPES.TECH) return false;
...
/**
 * The access levels that mean "Harmon staff" -- the tenant-wide half of the §3.5 union.
 *
 * DERIVED from SCOPE_BY_ACCESS_LEVEL rather than written out again, so a level added to
 * the scope table is automatically part of the union and the two cannot drift. A
 * hand-maintained copy is how "we made Executive Assistant a tenant role" becomes "the
 * dealer's mention picker stopped showing half of Harmon" six months later.
 */
export const TENANT_ACCESS_LEVELS = Object.freeze(
  Object.keys(SCOPE_BY_ACCESS_LEVEL).filter(
    (k) => SCOPE_BY_ACCESS_LEVEL[k] === SCOPES.TENANT
  )
);

/**
 * PostgREST or-expression values are comma/parenthesis delimited and optionally
 * double-quoted. Rather than escape, this REFUSES anything outside a conservative
 * alphabet -- Salesforce ids are alphanumeric and the access levels are our own
 * constants, so a value that fails this is not a value we recognize, and the filter
 * denies rather than emitting an expression whose parse we would be guessing at.
 */
const PG_SAFE = /^[A-Za-z0-9 _-]+$/;

/**
 * The `user` object filter (§3.5) -- a UNION, which is why it is not rowFilter().
 *
 *   tenant        every user in the tenant (today's behavior)
 *   dealer / own  (Dealer__c = mine) UNION (Access_Level__c in the tenant-wide set)
 *   none          deny
 *
 * @returns {{deny:true, code:string} | {deny:false, union:boolean, cache:Array<{column,value}>, cacheOr:string|null, soql:string}}
 *
 * `cache` is the AND-ed equality list (the tenant clause, and only that); `cacheOr` is
 * the PostgREST or-expression for the union half, or null for tenant scope. BOTH must be
 * applied. The OR group can only narrow, because it sits inside a conjunction with the
 * tenant clause.
 *
 * ⚠️ THE UNION IS WHY rowFilter() REFUSES `user`. A single column equality cannot express
 * "my dealer's people plus Harmon staff", and rowFilter returning the dealer equality
 * alone would silently hide every Harmon employee from a dealer's @-mention picker -- a
 * filter that is wrong in a way the caller cannot see. OBJECT_ACCESS.user carries
 * `unionFilter: true` for exactly that reason; these two functions are the pair.
 *
 * ⚠️ `dealer` AND `own` GET THE SAME UNION. `own` narrows RECORDS, not colleagues: a rep
 * who could only mention themselves could not ask anyone anything.
 *
 * ⚠️ ACTIVE-ONLY IS NOT AN ACCESS DECISION and is deliberately absent. GET /sf/users
 * returns active users because a dropdown of departed staff is useless, not because an
 * inactive user is protected. The caller keeps its own `Active__c` / `active` clause;
 * folding it in here would make an endpoint's presentation rule look like a security
 * control, and the next caller that legitimately wants inactive users would have to work
 * around the authorization module to get them.
 */
export function userFilter(access) {
  const a = ctx(access);
  if (a.scope === SCOPES.NONE) return { deny: true, code: DENY.MODULE_FORBIDDEN };
  // Same rule as rowFilter: no isolation key, no query. Never a dealer clause alone.
  if (a.tenantId === null) return { deny: true, code: DENY.MODULE_FORBIDDEN };
  if (!PG_SAFE.test(a.tenantId)) return { deny: true, code: DENY.MODULE_FORBIDDEN };

  const cache = [{ column: "client_sf_id", value: a.tenantId }];
  const tenantSoql = `Client__c = '${escapeSoqlValue(a.tenantId)}'`;

  if (a.scope === SCOPES.TENANT || a.scope === SCOPES.TECH) {
    // No narrowing beyond the tenant -- identical to what the endpoint serves today,
    // which is what makes the tenant-scope shadow comparison free (no second query).
    // A tech is staff: the directory is the @-mention list in the tech app's
    // Communications panel, and everyone in the tenant is fair to tag (amendment 7).
    return { deny: false, union: false, cache, cacheOr: null, soql: tenantSoql };
  }

  if (a.dealerId === null) return { deny: true, code: DENY.MODULE_FORBIDDEN };
  if (!PG_SAFE.test(a.dealerId)) return { deny: true, code: DENY.MODULE_FORBIDDEN };
  // An empty tenant-wide set would silently reduce the union to the dealer half. That
  // can only happen if the scope table is edited into nonsense, and the right answer to
  // nonsense here is the same as everywhere else in this file.
  if (TENANT_ACCESS_LEVELS.length === 0) return { deny: true, code: DENY.MODULE_FORBIDDEN };
  if (!TENANT_ACCESS_LEVELS.every((l) => PG_SAFE.test(l))) {
    return { deny: true, code: DENY.MODULE_FORBIDDEN };
  }

  const levels = TENANT_ACCESS_LEVELS.map((l) => `"${l}"`).join(",");
  const cacheOr = `dealer_sf_id.eq."${a.dealerId}",access_level.in.(${levels})`;
  const soqlLevels = TENANT_ACCESS_LEVELS.map((l) => `'${escapeSoqlValue(l)}'`).join(", ");
  ...
  return {
    ...
    cacheOr,
    soql:
      `${tenantSoql} AND (Dealer__c = '${escapeSoqlValue(a.dealerId)}' ` +
      `OR Access_Level__c IN (${soqlLevels}))`,
  };
}
...
/**
 * Does a `sundial_user_cache` ROW satisfy the §3.5 union? The in-memory twin of
 * userFilter(), for the single-read cache shortcut.
 *
 * rowMatchesFilter() cannot answer for `user`: it delegates to rowFilter(), which refuses
 * the object outright. Asking it anyway would return false for every user row and blank
 * the mention picker's single reads -- a denial produced by the wrong function, not by a
 * rule. So the union gets its own row check, and rowVisible() below is what callers use
 * so they never have to know which is which.
 *
 * A row missing `access_level` is not Harmon staff (the column is null for a user whose
 * Access_Level__c is unset, which resolves to `none` anyway). A row missing
 * `dealer_sf_id` is not in my dealer. Both fail closed.
 */
export function userRowMatches(access, row) {
  const f = userFilter(access);
  if (f.deny) return false;
  if (!row || typeof row !== "object") return false;
  if (!f.cache.every((c) => row[c.column] === c.value)) return false;
  // Tenant scope has no union half -- the tenant clause above is the whole filter.
  if (!f.union) return true;
  const a = ctx(access);
  if (row.dealer_sf_id != null && row.dealer_sf_id === a.dealerId) return true;
  return TENANT_ACCESS_LEVELS.includes(row.access_level);
}

/**
 * "May this context see this cache ROW of this object?" -- the ONE row check callers use.
 *
 * Dispatches on the object's own shape (`unionFilter`) so no Lambda has to remember that
 * `user` is answered by a different function. Forgetting that is not a hypothetical: it
 * is the exact mistake that would silently deny every user row on the cache shortcut.
 */
export function rowVisible(objectKey, access, row) {
  const def = own(OBJECT_ACCESS, objectKey);
  if (!def) return false;
  return def.unionFilter
    ? userRowMatches(access, row)
    : rowMatchesFilter(objectKey, access, row);
}
