...
test("scope tech may perform ONLY the service.tech.* actions (and the office may perform them too)", () => {
  const tech = ctxFor("Technician");
  for (const key of Object.keys(ACTION_SCOPES)) {
    assert.equal(canAction(key, tech), key.startsWith("service.tech."), key);
  }
  ...
  assert.deepEqual(actionsFor(tech), ["service.tech.self"]);
  assert.deepEqual(modulesFor(tech), []);
  // The generic reads stay shut: rowFilter and userFilter both deny a tech.
  assert.equal(rowFilter("servicecall", tech).deny, true);
  assert.equal(rowFilter("job", tech).deny, true);
  assert.equal(userFilter(tech).deny, true);
});
...
test("accessBlock for a technician: no modules, the tech action, scope `tech`", () => {
  ...
  assert.deepEqual(b.modules, []);
  assert.deepEqual(b.actions, ["service.tech.self"]);
});
...
test("userFilter FAILS CLOSED: none scope, no tenant, no dealer", () => {
  assert.deepEqual(userFilter(ctxFor("Technician")), {
    deny: true,
    code: DENY.MODULE_FORBIDDEN,
  });
