...
const MODULE_MATRIX = {
  // Service columns (estimate/job/servicecall/pricebookitem/serviceline/
  // serviceinvoice/servicepayment, D-072): an office/tech MODULE — tenant scopes
  // yes, sales scopes no (same reasoning as roofing). Technician stays all-"n":
  // the `tech` scope (2026-09-15) reads NOTHING through the generic routes — the
  // tech app has its own routes under service.tech.self, each scoped to the
  // caller's own calls. The matrix pins that the new scope did not leak here.
  // serviceplan / membership (Service Club, D-073) follow the Service columns exactly.
  //               customer solar roofing commercial po  po_credit estimate job servicecall pricebookitem serviceline serviceinvoice servicepayment serviceplan membership user
  Executive:      ["y",     "y",  "y",    "y",       "y", "y",     "y",     "y", "y",        "y",          "y",        "y",           "y",           "y",        "y",       "y"],
  Admin:          ["y",     "y",  "y",    "y",       "y", "y",     "y",     "y", "y",        "y",          "y",        "y",           "y",           "y",        "y",       "y"],
  Manager:        ["y",     "y",  "y",    "y",       "y", "y",     "y",     "y", "y",        "y",          "y",        "y",           "y",           "y",        "y",       "y"],
  "Sales Dealer": ["y",     "y",  "n",    "n",       "n", "n",     "n",     "n", "n",        "n",          "n",        "n",           "n",           "n",        "n",       "y"],
  "Sales Rep":    ["y",     "y",  "n",    "n",       "n", "n",     "n",     "n", "n",        "n",          "n",        "n",           "n",           "n",        "n",       "y"],
  Technician:     ["n",     "n",  "n",    "n",       "n", "n",     "n",     "n", "n",        "n",          "n",        "n",           "n",           "n",        "n",       "n"],
};
const OBJECT_ORDER = ["customer", "solar", "roofing", "commercial", "po", "po_credit", "estimate", "job", "servicecall", "pricebookitem", "serviceline", "serviceinvoice", "servicepayment", "serviceplan", "membership", "user"];
...
test("canReadObject FAILS CLOSED on an unknown object key, even for tenant scope", () => {
  ...
  // ("service"/"visit" were the D-065 keys; D-072 renamed them - they must now
  // fail closed like any other unknown key, so they sit in this list on purpose.)
  for (const key of ["asset", "service", "visit", "ticket", "sundial_customer__c", "", null, undefined, "__proto__"]) {
...
const SALES_ALLOWED_ACTIONS = [
  ...
  "notify.self", // D-074: everyone signed in may manage their own push devices
];

test("sales roles get EXACTLY the allowed actions and no others", () => {
  for (const level of ["Sales Rep", "Sales Dealer"]) {
...
test("every file-object key has list/upload/delete action rows (the Roofing 403 of 2026-09-11)", async () => {
  // lib/file-access.js's allowlist and ACTION_SCOPES are two lists that must agree: the
  // file Lambdas gate on `files.<key>.<verb>` BEFORE consulting the allowlist, and an
  // action key with no row is denied. Roofing and PO were in the allowlist without rows
  // for months, so their Files tabs answered 403 to everyone. Pin the agreement.
  const { FILE_OBJECT_ALLOWLIST } = await import("./file-access.js");
  for (const key of Object.keys(FILE_OBJECT_ALLOWLIST)) {
    if (key === "user") continue; // no Files tab on users today
    for (const verb of ["list", "upload", "delete"]) {
      assert.ok(ACTION_SCOPES[`files.${key}.${verb}`], `missing ACTION_SCOPES row files.${key}.${verb}`);
    }
  }
  // And the service keys are office-only, like every service action.
  for (const key of ["estimate", "job", "servicecall", "roofing", "po"]) {
    for (const level of ["Sales Rep", "Sales Dealer", "Technician"]) {
      assert.equal(canAction(`files.${key}.list`, ctxFor(level)), false, `${level} x files.${key}.list`);
    }
    assert.equal(canAction(`files.${key}.list`, ctxFor("Manager")), true);
  }
});
...
test("scope tech may perform ONLY the service.tech.* actions + texting the customer (and the office may perform them too)", () => {
  const tech = ctxFor("Technician");
  const TECH_ACTIONS = new Set(["service.tech.self", "service.tech.read", "service.sms.send", "notify.self"]);
  for (const key of Object.keys(ACTION_SCOPES)) {
    assert.equal(canAction(key, tech), TECH_ACTIONS.has(key), key);
  }
  assert.equal(canAction("service.tech.self", ctxFor("Admin")), true);
  assert.equal(canAction("service.tech.self", ctxFor("Sales Rep")), false);
  assert.deepEqual(actionsFor(tech), ["service.sms.send", "service.tech.self", "service.tech.read", "notify.self"]);
  assert.deepEqual(modulesFor(tech), []);
  // The generic record reads stay shut (rowFilter denies a tech); the user directory —
  // the @-mention list — is open to staff, and a tech is staff.
  assert.equal(rowFilter("servicecall", tech).deny, true);
  assert.equal(rowFilter("job", tech).deny, true);
  assert.equal(userFilter(tech).deny, false);
});
...
test("accessBlock for a `none` user is empty, not absent", () => {
  ...
  const b = accessBlock(ctxFor("Sales Rep", { dealer: null }));
  assert.equal(b.scope, SCOPES.NONE);
  assert.deepEqual(b.modules, []);
  assert.deepEqual(b.actions, []);
});

test("accessBlock for a technician: no modules, the tech action, scope `tech`", () => {
  const b = accessBlock(ctxFor("Technician"));
  assert.equal(b.scope, SCOPES.TECH);
  assert.deepEqual(b.modules, []);
  assert.deepEqual(b.actions, ["service.sms.send", "service.tech.self", "service.tech.read", "notify.self"]);
});
...
// ---------------------------------------------------------------------------
// 9. userFilter — the §3.5 union (Phase 2)
// ---------------------------------------------------------------------------
//
// This is the one filter in the module that is NOT a conjunction of equalities, and the
// tests are written around the two ways a union goes wrong: losing the dealer half (the
// rep sees all of Harmon plus nobody they work with) or losing the tenant half (the
// dealer half leaks across tenants the day a second tenant exists).

test("userFilter: tenant scope is the tenant clause ALONE — today's behavior", () => {
  const f = userFilter(ctxFor("Admin"));
  assert.equal(f.deny, false);
  assert.equal(f.union, false);
  assert.deepEqual(f.cache, [{ column: "client_sf_id", value: TENANT }]);
  assert.equal(f.cacheOr, null);
  assert.equal(f.soql, `Client__c = '${TENANT}'`);
});

test("userFilter: dealer scope -> my dealer's people UNION Harmon staff", () => {
  const f = userFilter(ctxFor("Sales Dealer"));
  assert.equal(f.deny, false);
  assert.equal(f.union, true);
  // The tenant clause is an AND, never part of the OR group.
  assert.deepEqual(f.cache, [{ column: "client_sf_id", value: TENANT }]);
  assert.equal(
    f.cacheOr,
    `dealer_sf_id.eq."${DEALER}",access_level.in.("Executive","Admin","Manager")`
  );
  assert.equal(
    f.soql,
    `Client__c = '${TENANT}' AND (Dealer__c = '${DEALER}' ` +
      `OR Access_Level__c IN ('Executive', 'Admin', 'Manager'))`
  );
});

test("userFilter: `own` gets the SAME union as `dealer` — own narrows records, not colleagues", () => {
  // A rep who could only see themselves could not @-mention anyone. §3.5 gives both
  // sales scopes the identical set; only the RECORD filter differs between them.
  assert.deepEqual(userFilter(ctxFor("Sales Rep")), userFilter(ctxFor("Sales Dealer")));
});

test("userFilter: the tenant clause is in EVERY branch", () => {
  for (const level of ["Executive", "Admin", "Manager", "Sales Dealer", "Sales Rep"]) {
    const f = userFilter(ctxFor(level));
    assert.equal(f.deny, false, level);
    assert.ok(
      f.cache.some((c) => c.column === "client_sf_id" && c.value === TENANT),
      `${level}: no tenant clause`
    );
    assert.ok(f.soql.startsWith(`Client__c = '${TENANT}'`), `${level}: no tenant SOQL`);
  }
});

test("userFilter FAILS CLOSED: none scope, no tenant, no dealer — and a tech sees the whole directory (they are staff)", () => {
  assert.deepEqual(userFilter(ctxFor("Technician")), userFilter(ctxFor("Admin")));
  assert.equal(userFilter(ctxFor("Technician")).deny, false);
  assert.deepEqual(userFilter(ctxFor(null)), { deny: true, code: DENY.MODULE_FORBIDDEN });
  assert.deepEqual(userFilter(undefined), { deny: true, code: DENY.MODULE_FORBIDDEN });
  // A sales role with no tenant: never the dealer clause alone (same rule as rowFilter).
  assert.deepEqual(
    userFilter(resolveScope(userWith("Sales Dealer"), null)),
    { deny: true, code: DENY.MODULE_FORBIDDEN }
  );
  // A sales role with no dealer resolves to `none` before it gets here, but assert the
  // filter refuses it directly too — the deny must not depend on resolveScope running.
  assert.deepEqual(
    userFilter({ scope: SCOPES.DEALER, userId: USER, dealerId: null, tenantId: TENANT }),
    { deny: true, code: DENY.MODULE_FORBIDDEN }
  );
});

test("userFilter REFUSES a value it cannot safely put in a PostgREST or-expression", () => {
  // The or-expression is comma/paren delimited. Rather than escape, unsafe input denies.
  // A Salesforce id cannot contain these; a value that does is not one we recognize.
  for (const bad of ['a0Y",x', "a0Y,b", "a0Y)b", "a0Y(b", "a0Y.b"]) {
    assert.deepEqual(
      userFilter({ scope: SCOPES.DEALER, userId: USER, dealerId: bad, tenantId: TENANT }),
      { deny: true, code: DENY.MODULE_FORBIDDEN },
      bad
    );
    assert.deepEqual(
      userFilter({ scope: SCOPES.DEALER, userId: USER, dealerId: DEALER, tenantId: bad }),
      { deny: true, code: DENY.MODULE_FORBIDDEN },
      bad
    );
  }
...
test("TENANT_ACCESS_LEVELS is DERIVED from the scope table, not a second copy", () => {
  // The failure this prevents: a level added to SCOPE_BY_ACCESS_LEVEL as tenant scope
  // while the union's staff list is edited separately (or not at all), so a dealer's
  // picker silently stops showing part of Harmon.
  const derived = Object.keys(SCOPE_BY_ACCESS_LEVEL).filter(
    (k) => SCOPE_BY_ACCESS_LEVEL[k] === SCOPES.TENANT
  );
  assert.deepEqual([...TENANT_ACCESS_LEVELS], derived);
  assert.deepEqual([...TENANT_ACCESS_LEVELS], ["Executive", "Admin", "Manager"]);
  // Every level in the union must actually BE tenant scope — no sales level may leak in.
  for (const level of TENANT_ACCESS_LEVELS) {
    assert.equal(SCOPE_BY_ACCESS_LEVEL[level], SCOPES.TENANT, level);
  }
});

test("userFilter and rowFilter disagree about `user` ON PURPOSE", () => {
  // rowFilter refuses it (an equality cannot express the union); userFilter answers it.
  // If rowFilter ever starts returning a filter for `user`, that is a caller silently
  // getting the dealer half without the staff half.
  const rep = ctxFor("Sales Rep");
  assert.equal(rowFilter("user", rep).deny, true);
  assert.equal(userFilter(rep).deny, false);
  assert.equal(OBJECT_ACCESS.user.unionFilter, true);
});
