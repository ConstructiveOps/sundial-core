...
import {
  ...
  ATTR_GATE, syncProjectAttributes, JOBTYPE_VALUE, JOBTYPE_ATTRIBUTE_ID,
...
test("KW is the system size and JOBTYPE comes from the caller", () => {
  assert.equal(asMap(LIVE).KW, "12.760");
  assert.equal(asMap(LIVE, { jobType: JOBTYPE_VALUE }).JOBTYPE, "RS");
  // Without it, JOBTYPE is omitted rather than guessed. The builder stays value-agnostic
  // even though both callers now pass the same constant: what it must never do is invent
  // one, and the omission is what the merge relies on.
...
function fakeAcumatica({
  ...
  writeStatus = 200,
  readStatus = 200,
  manager = null,      // ProjectProperties.ProjectManager as it stands before the write
  dropManager = false, // 200, but the manager never lands — the silent-discard shape
} = {}) {
  const store = { ...existing };
  const state = { manager };
  const allowed = defined; // null = the template defines everything
  const calls = { puts: [], gets: 0, queries: [] };
  return {
    calls,
    store,
    state,
    deps: {
      getAcumaticaEntity: async (_entity, query) => {
        ...
        return {
          ...
          data: [{
            ...
            // Only present because the caller expanded it; see the EXPAND note in the
            // module. A fake that always returned it would hide that requirement.
            ProjectProperties: /ProjectProperties/.test(query?.$expand ?? "")
              ? { ProjectManager: state.manager ? { value: state.manager } : {} }
              : undefined,
          }],
        };
      },
      putAcumaticaEntity: async (_entity, body) => {
        ...
        const sentManager = body.ProjectProperties?.ProjectManager?.value;
        if (sentManager && !dropManager) state.manager = sentManager;
...
test("JOBTYPE is sent when the caller supplies it, and it is the ValueID", async () => {
  // ~~The budget push cannot know RS vs RSDC authoritatively.~~ Superseded 2026-09-08
  // (D31 / D-067): JOBTYPE does not hold RS-vs-RSDC. Its allowed ValueIDs are
  // CE CS EV RE RS SE — there is no RSDC job type, and every live RSDC-template project
  // carries RS. So it is a constant, nothing is inferred, and the push worker sends it.
  const f = fakeAcumatica({ existing: { JOBTYPE: "RE" } });
  await syncProjectAttributes("R261065", LIVE, { deps: f.deps, jobType: JOBTYPE_VALUE });
  const sent = f.calls.puts[0].Attributes.find((a) => a.AttributeID.value === "JOBTYPE");
  assert.ok(sent, "JOBTYPE must be in the body");
  assert.equal(sent.Value.value, "RS");
  assert.notEqual(sent.Value.value, "Residential Solar", "the label is discarded with a 200");
  assert.equal(f.store.JOBTYPE, "RS");
});

test("JOBTYPE is still omitted when the caller does NOT supply it", async () => {
  // The attribute-only path (legacy / hand-budgeted projects) passes no jobType, and the
  // merge then leaves whatever is there alone. Its safety claim is that it writes only
  // what it was asked to; widening it is a separate decision.
...
// ---------------------------------------------------------------------------
// The project manager rides along on the same PUT (2026-09-08)
// ---------------------------------------------------------------------------

test("a supplied project manager goes to ProjectProperties, in the SAME PUT", async () => {
  // Same write, same project, same proof. A second round trip would be a second thing
  // that could half-succeed.
  const f = fakeAcumatica();
  const r = await syncProjectAttributes("R261065", LIVE, {
    deps: f.deps, jobType: JOBTYPE_VALUE, projectManager: "E00675",
  });
  assert.equal(f.calls.puts.length, 1, "one PUT, not two");
  assert.deepEqual(f.calls.puts[0].ProjectProperties, { ProjectManager: { value: "E00675" } });
  assert.equal(f.state.manager, "E00675");
  assert.equal(r.ok, true);
  assert.equal(r.projectManager, "E00675");
  assert.equal(r.managerVerified, true);
});

test("NO project manager means the key is ABSENT — it must never clear one", async () => {
  // Blank field, unmapped name and ambiguous multi-select all arrive here as null: three
  // ways of not knowing, none of them a reason to wipe a manager someone assigned. There
  // is one slot, so a wrong clear is total.
  for (const none of [undefined, null, "", "   "]) {
    const f = fakeAcumatica({ manager: "E01177" });
    await syncProjectAttributes("R261065", LIVE, { deps: f.deps, projectManager: none });
    assert.equal("ProjectProperties" in f.calls.puts[0], false, `sent for ${JSON.stringify(none)}`);
    assert.equal(f.state.manager, "E01177", "the existing manager must survive untouched");
  }
});

test("the verifying re-read EXPANDS ProjectProperties — otherwise it reads undefined", async () => {
  // Confirmed live: the object is simply absent from the response without the expand, so
  // a verification without it would report every successful write as a failure.
  const f = fakeAcumatica();
  await syncProjectAttributes("R261065", LIVE, { deps: f.deps, projectManager: "E00675" });
  for (const q of f.calls.queries) {
    assert.match(q.$expand, /ProjectProperties/);
    assert.match(q.$expand, /Attributes/);
  }
});

test("a manager accepted with a 200 and then DROPPED is caught by the re-read", async () => {
  // The same hazard the attributes have. A status code is not evidence that a write
  // happened; only a re-read is.
  const f = fakeAcumatica({ dropManager: true });
  const r = await syncProjectAttributes("R261065", LIVE, {
    deps: f.deps, jobType: JOBTYPE_VALUE, projectManager: "E00675",
  });
  assert.equal(r.ok, false);
  assert.equal(r.action, "unverified");
  assert.equal(r.managerVerified, false);
  assert.match(r.message, /project manager did not land/);
  assert.match(r.message, /E00675/);
});

test("a manager alone is worth a PUT even when every attribute is blank", async () => {
  // An empty record used to short-circuit to nothing_to_write. That would have made the
  // refresh a no-op on exactly the legacy-shaped records it is most useful for.
  const f = fakeAcumatica();
  const r = await syncProjectAttributes("R261065", {}, { deps: f.deps, projectManager: "E01177" });
  assert.equal(r.ok, true);
  assert.equal(r.action, "synced");
  assert.equal(f.calls.puts.length, 1);
  assert.equal("Attributes" in f.calls.puts[0], false, "no attributes means no Attributes key");
  assert.deepEqual(f.calls.puts[0].ProjectProperties, { ProjectManager: { value: "E01177" } });
  assert.equal(f.state.manager, "E01177");
});

test("nothing to say at all is still nothing_to_write, with no PUT", async () => {
  const f = fakeAcumatica();
  const r = await syncProjectAttributes("R261065", {}, { deps: f.deps, projectManager: null });
