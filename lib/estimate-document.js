...
export function buildEstimateModel({ estimate, lines, totals, brand = {}, options = {} }) {
  ...
  return {
    ...
    number,
    version,
    brand: { companyName: b.companyName || "Estimate", lines: brandLines, termsUrl: b.termsUrl || "", footerNote: b.footerNote || "", accentColor: b.accentColor },
    metaLines,
    customerLines,
...
export function buildInvoiceModel({ invoice, job, estimate, lines, payments = [], brand = {}, options = {} }) {
  ...
  const brandLines = [b.tagline, b.addressLine, [b.phone, b.email].filter(Boolean).join(" · "), b.licenseLine].filter(Boolean);
  ...
  return {
    ...
    number,
    version: 0,
    brand: { companyName: b.companyName || "Invoice", lines: brandLines, termsUrl: b.termsUrl || "", footerNote: b.footerNote || "", accentColor: b.accentColor },
    metaLines,
    customerLines,
    scope: est.Scope_Summary__c || j.Customer_Summary__c || "",
    columns: { description: "Description", qty: "Qty", unit: showUnit ? "Unit" : "", amount: "Amount" },
    rows,
    totalRows,
...
<body><div class="page">
  ...
  <div class="two">
    ...
    ${m.scope ? `<div><h2>Scope of work</h2><div class="scope">${esc(m.scope)}</div></div>` : ""}
  </div>
  ...
  <footer>
    ...
    ${m.brand.termsUrl ? `<p>Terms and conditions: <a href="${esc(m.brand.termsUrl)}">${esc(m.brand.termsUrl)}</a></p>` : ""}
