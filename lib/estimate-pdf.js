...
const DARK = rgb(0.16, 0.16, 0.18);
/** The logo's box in the header: at most this tall, at most half the content width. */
export const LOGO_MAX_H = 64;
export const LOGO_MAX_W = 220;

/**
 * Draw the brand's logo at the top-left of the header (2026-09-24). Returns the height
 * used, 0 when there is no logo or it cannot be embedded — the caller prints the name.
 * Shared with the job-report painter.
 */
...
/**
 * The footer's links (terms, the Service Club) as lines to print: the label bold, the
 * copy and the URL wrapped in grey. `{ text, bold, gap }` — `gap` adds air after a link.
 * Shared with the job-report painter.
 */
...
export async function renderEstimatePdf(model) {
  ...
  const headerTop = y;
  // The logo, when the brand has one the loader could fetch (2026-09-24); else the name.
  const logoH = await drawLogo(doc, page, model.brand, MARGIN, headerTop);
  if (logoH > 0) y -= logoH + 6;
  else {
  ...
  const leftBottom = y;
  y = headerTop;
  textRight(model.docLabel || "Estimate", PAGE.width - MARGIN, 9, { color: GREY });
  y -= 13;
  ...
  const blockTop = y;
  sectionLabel(model.customerLabel || "Prepared for");
  ...
  if (model.scope) {
    ...
    y -= 14;
    page.drawText(safe((model.scopeLabel || "Scope of work").toUpperCase()), { x, y: y - 8, size: 8, font, color: GREY });
    y -= 12;
  ...
  if (model.accept) {
    ...
    y -= 10;
    text(model.accept.label ? `${model.accept.label} online:` : "Approve online:", MARGIN, 10, { bold: true });
    y -= 13;
  ...
  const linkLines = footerLinkLines(model.brand, font, bold, CONTENT_W);
  ensure((footerLines.length + linkLines.length) * 11 + 20);
  y -= 16;
  rule(LIGHT, 1);
  y -= 8;
  ...
  // The terms and the Service Club (2026-09-24): label in bold, the copy, the URL.
  for (const l of linkLines) {
    ensure(11);
    text(l.text, MARGIN, 8, { bold: l.bold, color: l.bold ? DARK : GREY });
    y -= l.gap ? 14 : 11;
  }
