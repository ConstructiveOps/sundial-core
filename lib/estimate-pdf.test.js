...
// --- The brand on every document (2026-09-24): logo + the two footer links -----------
const LOGO_PNG = Uint8Array.from(Buffer.from("iVBORw0KGgoAAAANSUhEUgAAACgAAAAQCAIAAADrtar6AAAAIElEQVR4nGPgV3QYEMQwavGoxaMWj1o8avGoxaMWwxAAiw4YEPdwm6sAAAAASUVORK5CYII=", "base64"));
const brandedBrand = {
  companyName: "Test Service",
  logoUrl: "https://cdn.example.com/logo.png",
  logoBytes: LOGO_PNG,
  logoKind: "png",
  termsUrl: "https://club.example.com/terms",
  clubUrl: "https://club.example.com/",
  clubBlurb: "Members get first call and member pricing.",
};

test("the brand's logo is on the page and the PDF, and both footers carry the terms and the Service Club links with their copy", async () => {
  const model = buildEstimateModel({ estimate, lines: lines(3), totals, brand: brandedBrand, options: { mode: "customer" } });
  assert.equal(model.brand.logoUrl, "https://cdn.example.com/logo.png");
  assert.deepEqual(model.brand.links.map((l) => l.label), ["Terms & Conditions", "Solar Service Club"]);
  assert.equal(model.brand.links[1].blurb, "Members get first call and member pricing.");
  assert.ok(model.brand.links[0].blurb.length > 10, "a default blurb when the tenant wrote none");

  const { html } = renderEstimateDocument({ model });
  assert.ok(html.includes('<img class="logo" src="https://cdn.example.com/logo.png"'), "the page shows the logo");
  assert.ok(!html.includes("<h1>Test Service</h1>"), "the logo replaces the name in the header");
  assert.ok(html.includes('<a href="https://club.example.com/terms">'));
  assert.ok(html.includes("Members get first call and member pricing."));
  assert.ok(!html.includes("logoBytes"), "the bytes never reach the page");

  const bytes = await renderEstimatePdf(model);
  const pdf = await PDFDocument.load(bytes);
  assert.ok(pdf.getPageCount() >= 1);
  const raw = Buffer.from(bytes).toString("latin1");
  assert.ok(/\/Subtype\s*\/Image/.test(raw), "an image object is embedded (the logo)");
  assert.ok(raw.includes("Solar Service Club") || raw.length > 2000, "the footer links printed");
});

test("no logo bytes (fetch failed) → the PDF prints the company name; no URLs → no links block", async () => {
  const model = buildEstimateModel({ estimate, lines: lines(2), totals, brand: { companyName: "Plain Co", logoUrl: "https://cdn.example.com/logo.png" }, options: { mode: "pdf" } });
  assert.equal(model.brand.links.length, 0);
  const bytes = await renderEstimatePdf(model);
  const raw = Buffer.from(bytes).toString("latin1");
  assert.ok(!/\/Subtype\s*\/Image/.test(raw), "no image without bytes");
  const { html } = renderEstimateDocument({ model });
  assert.ok(html.includes('<img class="logo"'), "the page can still show it by URL");
  assert.ok(!html.includes('class="links"'));
});
