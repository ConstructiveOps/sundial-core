...
// Every key here needs a matching `files.<key>.*` row in lib/access.js ACTION_SCOPES,
// or the action gate denies it before this list is even consulted.
export const FILE_OBJECT_ALLOWLIST = {
  ...
  // Service module (D-072, 2026-09-11). The estimate PDF lands under the estimate;
  // job files and service-call photos (photos/{serviceCallId}/) under the job.
  estimate: "Sundial_Estimate__c",
  job: "Sundial_Service_Job__c",
  servicecall: "Sundial_Service_Call__c",
};
...
// Is there already a metadata row for this exact S3 key? Returns the row's id, or
// null.
//
// WHY THIS EXISTS: S3 keys are deterministic, so re-running a writer (a webhook
// redelivery, a retried copy) overwrites the OBJECT harmlessly but would INSERT a
// second metadata row — the file then shows twice in the Files tab with no way for a
// user to tell which is which. Callers that can legitimately run twice should check
// this before registering.
//
// Soft-deleted rows count as existing: a user deleted the file on purpose, and a
// re-upload should not silently resurrect it as a second row.
//
// Returns null on a Supabase error as well as on "not found" — the caller is then
// registering best-effort anyway, and a duplicate row is a smaller problem than a
// dropped one.
export async function findFileMetadataByKey(supabase, s3Key) {
  const { data, error } = await supabase
    .from(FILE_METADATA_TABLE)
    .select("id")
    .eq("s3_key", s3Key)
    ...
    .maybeSingle();
  if (error) {
    console.warn(`file metadata lookup failed for ${s3Key}: ${error.message}`);
    return null;
  }
  return data?.id ?? null;
}
