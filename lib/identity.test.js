...
test("Technician resolves to tech (D-072 amendment 7 — the tech app's own scope, no dealer needed)", async () => {
  // Was `none` until 2026-09-15. Tim, 2026-09-17: techs are Harmon employees; the D-064
  // sales-rep / dealer process does not govern them. A tech has no dealer and needs none.
  ctx.record = sfRecord({ Access_Level__c: "Technician", Dealer__c: null, Dealer__r: null });
  const id = await resolveIdentity("Bearer x");
  assert.equal(id.access.scope, "tech");
