...
// In-flight token request, so N concurrent callers on a cold container share ONE
// JWT bearer round trip instead of each opening their own. This matters now that a
// single request can fan out into several parallel Salesforce queries (see
// refetchByIds in sundial-sf-query): without it, a cold invoke would fire that many
// simultaneous /services/oauth2/token calls for the same integration user.
//
// The promise is CLEARED on settle — both success and failure. A rejected promise
// left in place would be re-awaited by every later caller on this warm container
// and turn one transient auth blip into a permanently broken container.
...
export async function getSalesforceToken(opts = {}) {
  ...
  // Coalesce concurrent refreshes onto one request (see inFlightTokenRequest).
  if (!inFlightTokenRequest) {
    inFlightTokenRequest = requestNewToken().finally(() => {
      inFlightTokenRequest = null;
    });
  }
  const fresh = await inFlightTokenRequest;
  return {
...
// ---------------------------------------------------------------------------
// Object describe (module-scope cached, with TTL)
// ---------------------------------------------------------------------------

// Why a TTL (D-045): a describe carries the field set AND the integration user's
// per-field FLS. A warm container that cached it before a permission-set change or
// a new field keeps serving stale metadata, which silently drops fields from
// writes. Five minutes bounds that without needing a redeploy.
const DESCRIBE_TTL_MS = 5 * 60 * 1000;
const describeCache = new Map(); // sfObject -> { meta, fetchedAt }

/**
 * Fetch an object's describe, cached with a TTL. Callers use it to check that a
 * field exists (and is writable) before including it in a query or a PATCH.
 *
 * @param {string} sfObject - e.g. "Sundial_Customer__c"
 * @param {{ forceRefresh?: boolean }} [opts]
 * @returns {Promise<object>} the raw describe payload (fields[], etc.)
 */
export async function describeObject(sfObject, opts = {}) {
  const cached = describeCache.get(sfObject);
  if (!opts.forceRefresh && cached && Date.now() - cached.fetchedAt < DESCRIBE_TTL_MS) {
    return cached.meta;
  }
  async function run(forceRefresh) {
    const { access_token, instance_url } = await getSalesforceToken({ forceRefresh });
  ...
  if (resp.status === 401) resp = await run(true); // expired session -> refresh once
  if (!resp.ok) {
    const text = await resp.text();
    const err = new Error(`Salesforce describe ${sfObject} failed (${resp.status})`);
...
/**
 * UPSERT a record by an External ID field (REST PATCH on
 * /sobjects/{obj}/{extIdField}/{value}).
 *
 * WHY THIS EXISTS: it is the only way to make "create if absent, update if
 * present" atomic in Salesforce. A SELECT-then-create has a race window that two
 * concurrent deliveries of the same webhook will eventually find, producing
 * duplicate customers for one Aurora project. The upsert makes the external id the
 * uniqueness authority, so concurrent callers converge on ONE record.
 *
 * The target field must be flagged External ID in Salesforce, otherwise Salesforce
 * rejects the call. If more than one record already carries the value, Salesforce
 * answers 300 Multiple Choices — surfaced here as `.sfStatus = 300` so the caller
 * can treat ambiguity as permanent rather than retrying into the same wall.
 *
 * @param {string} sfObject
 * @param {string} externalIdField - e.g. "Aurora_Project_ID__c"
 * @param {string} externalIdValue
 * @param {object} fields - do NOT include the external id field itself
 * @returns {Promise<{ ok: true, id: string, created: boolean }>}
 * @throws Error with .sfStatus/.sfBody on a non-2xx.
 */
export async function sfUpsertRecord(sfObject, externalIdField, externalIdValue, fields) {
  const path =
    `/services/data/${SF_API_VERSION}/sobjects/${sfObject}/` +
    `${encodeURIComponent(externalIdField)}/${encodeURIComponent(externalIdValue)}`;
  ...
  if (!resp.ok) {
    const err = new Error(
      resp.status === 300
        ? `Salesforce upsert ambiguous (300): more than one ${sfObject} already has ${externalIdField} = "${externalIdValue}".`
        : `Salesforce upsert failed (${resp.status})`
    );
    err.sfStatus = resp.status;
    err.sfBody = text;
    throw err;
  ...
  try {
    ...
    /* 204 No Content on update in older API behavior — handled below */
  }

  if (data?.id) {
    return { ok: true, id: data.id, created: data.created === true };
  }

  // No body (a 204 update): the record exists but we weren't told its id. Look it
  // up by the same external id so callers always get one back.
  const rows = await sfQuery(
    `SELECT Id FROM ${sfObject} WHERE ${externalIdField} = '${soqlEscapeString(externalIdValue)}' LIMIT 1`
  );
  return { ok: true, id: rows?.[0]?.Id ?? null, created: false };
}
