<#
.SYNOPSIS
    Wires the POST /projects/{recordId}/budget/push route on the Sundial REST API
    (API Gateway 5sktfwldh1, region us-west-1, stage prod) to the
...
.DESCRIPTION
    ...
    MOCK OPTIONS returning the CORS headers, per docs/api-endpoints.md). The
    /projects/{recordId}/budget resources already exist (created by the recalc
    route); only the final 'push' resource is new here.

    Auth is enforced INSIDE the Lambda (resolveIdentity verifies the Supabase JWT),
    so the gateway method uses authorization-type NONE — matching the other Sundial
    routes (copy-to-solar, budget/recalc).

    PRECONDITIONS (the script verifies these and stops if unmet):
      1. The sundial-acumatica-budget-push Lambda must already exist (add-permission
         needs it).
    ...
    Path is built as nested resources:
      /projects  ->  /projects/{recordId}  ->  .../budget  ->  .../push (POST + OPTIONS)

.EXAMPLE
    .\scripts\wire-budget-push-route.ps1            # interactive, prompts before prod deploy
    .\scripts\wire-budget-push-route.ps1 -Yes       # non-interactive
#>
[CmdletBinding()]
param([switch]$Yes)
...
Write-Host "==> Ensuring resource path /projects/{recordId}/budget/push" -ForegroundColor Cyan
...
$push     = Ensure-Resource $budget "push"
...
$srcArn = "arn:aws:execute-api:${Region}:${AcctId}:${ApiId}/*/POST/projects/*/budget/push"
aws lambda add-permission --function-name $Fn --region $Region `
    --statement-id "apigw-budget-push" --action "lambda:InvokeFunction" `
...
aws apigateway create-deployment --rest-api-id $ApiId --region $Region --stage-name $Stage `
    --description "Add POST /projects/{recordId}/budget/push -> $Fn" | Out-Null
Write-Host "SUCCESS: route live at https://$ApiId.execute-api.$Region.amazonaws.com/$Stage/projects/{recordId}/budget/push" -ForegroundColor Green
